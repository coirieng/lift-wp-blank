...
if ( ! class_exists('WOW_Editor_Management')){
	class WOW_Editor_Management {

		private $_post_id;

		public function __construct() {
			add_action( 'admin_action_wow_editor', array($this, 'init') );
		...
		public function redirect_to_frontend_editor(){
			if ( ! isset( $_GET['wow_editor'] ) ) {
				return;
			}

			$post_id = get_the_ID();
			wp_redirect( wow_helper()->get_editor_url($post_id) );
			die;
		}
		...
		public function init() {
			...
			wow_helper()->setup_post_data($this->_post_id);

			//add_filter( 'show_admin_bar', '__return_false' );
			$this->mark_post_as_wow_editor_content();
		...
		private function load_frontend_template(){
			include WOW_DIR_PATH.'inc/editor-main.php';
		}
		...
		public function template_data_footer_add(){
			...
			if ( class_exists('WOW_Ajax')){
				echo '<script id="wow-tmp-common-css" type="x-tmpl-lodash">';
					echo WOW_Ajax::get_content_common_css();
				echo '</script>';
			}

			$global = new WOW_Base_Data();
		 ...
		 * @param int $post_id
		 */
		public function mark_post_as_wow_editor_content($post_id = 0){
			if ( ! $post_id){
				$post_id = get_the_ID();
			}
			update_post_meta($post_id, '_is_wow_editor', true);
		...
		public function print_edit_with_wp_pagebuilder_button(){
            ...
            $currentEditor = get_post_meta($post->ID, '_wow_current_post_editor', true);

			?>
			<script id="wow-edit-with-btn-in-gutenberg-toolbar" type="text/html">
				<div class="wow-editor-warper">
					<a href="<?php echo wow_helper()->get_editor_url($post->ID); ?>" class="components-button  is-button is-primary is-large edit-with-wow-builder" style="display: <?php echo $currentEditor !== 'wow_builder_activated'? 'flex':'none'; ?>;"><?php _e('Edit With WOW Page Builder','wow-pagebuilder');?></a>
		...
		public function save_by_wp( $post_ID, $data){
			...
			if ($is_guttenBlock){
	            ...
	            if ($previousPost->post_content != $data['post_content'] ){
		            update_post_meta($post_ID, '_wow_current_post_editor', 'gutenberg');
	            }
            }

		    if ( ! empty($_POST['action']) && $_POST['action'] === 'editpost'){
                $previousPost = $wpdb->get_row("SELECT * from {$wpdb->posts} WHERE ID = {$post_ID} ");
                $shouldMakeClassicEditor = apply_filters('wow_should_view_classic_editor_content', false);

                if (strlen(sanitize_text_field($previousPost->post_content)) != strlen(sanitize_text_field($data['post_content'])) ){
                    update_post_meta($post_ID, '_wow_current_post_editor', 'wow_default_editor');
                }
