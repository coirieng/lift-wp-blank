...
if ( ! class_exists('WOW_Layout_Generator')){
	class WOW_Layout_Generator{
		...
		public function row_stretch_class($setting = array(), $affected_row = 'row-parent'){
			$row_dynamic_class = " wow-row-{$setting['id']} ";
			if ($affected_row === 'row-parent'){
			    ...
			    if ( ! isset($setting['settings']['row_screen']) || $setting['settings']['row_screen'] === 'row-default'){
			        return $row_dynamic_class. ' wow-container-row-default';
                }
            }
            return '';
        }
			...
			foreach ( $settings as $setting ){
				...
				if ( ! empty($setting['settings']['row_animation'])){
					...
					if ( ! empty($animation_settings->itemOpen) && $animation_settings->itemOpen){
						$animation_class = " wow-wow {$animation_settings->name} ";
				...
				if ( ! empty($setting['settings']['row_screen']) ){
					if( $setting['settings']['row_screen']=== 'row-container-stretch' ){
						$full_container_stretch[] = 'wow-container-full';
					}else if( $setting['settings']['row_screen']=== 'container-stretch-no-gutter' ){
						array_push( $full_container_stretch, 'wow-container-full', 'wow-contaner-no-gutter' );
					}
                }
				...
				?>
                <div <?php echo $row_css_id; ?> class="wow-row-parent <?php echo $parent_row_class.$animation_class.$row_css_class; ?>" <?php echo $animation_attr; ?> >
					...
					<?php if( !empty($setting['settings']['row_shape'])  && $setting['settings']['row_shape']['itemOpenShape'] == 1 && $setting['settings']['row_screen'] != 'row-default' ){ ?>
                        <div class="wow-shape-container wow-top-shape">
							<?php echo file_get_contents( WOW_DIR_URL.'assets/shapes/'.$setting['settings']['row_shape']['shapeStyle'].'.svg' ); ?>
						</div>
					<?php } ?>
					<?php if( !empty($setting['settings']['row_shape_bottom'])  && $setting['settings']['row_shape_bottom']['itemOpenShape'] == 1 && $setting['settings']['row_screen'] != 'row-default' ){ ?>
                        <div class="wow-shape-container wow-bottom-shape">
							<?php echo file_get_contents( WOW_DIR_URL.'assets/shapes/'.$setting['settings']['row_shape_bottom']['shapeStyle'].'.svg' ); ?>
						</div>
					<?php } ?>
					...
					<div class="wow-container <?php echo $row_container_class.implode( ' ', $full_container_stretch ); ?>">
						<?php if( !empty($setting['settings']['row_shape'])  && $setting['settings']['row_shape']['itemOpenShape'] == 1 && $setting['settings']['row_screen'] == 'row-default' ){ ?>
							<div class="wow-shape-container wow-top-shape">
								<?php echo file_get_contents( WOW_DIR_URL.'assets/shapes/'.$setting['settings']['row_shape']['shapeStyle'].'.svg' ); ?>
							</div>
								...
								foreach ($setting['columns'] as $col){
									...
									if ( isset($col['settings']['col_animation']['itemOpen'])){
										...
										if ($enable_col_animation) {
											$col_animation_class 	= isset( $col_animation['name'] ) ? 'wow-wow ' . $col_animation['name'] : '';
									...
									?>
                                    <div <?php if ($col_css_id) echo 'id="'.$col_css_id.'"'; ?> class="wow-column-parent wow-column-parent-view wow-col-<?php echo $col['id']; ?> <?php echo $col_css_class; ?> <?php echo $col_animation_class; ?>" <?php echo $col_animation_data;?> >
															...
															if ( isset($addon['visibility']) && $addon['visibility'] == true ) {
															    ...
															    if (isset($addon['settings']['addon_animation'])){
                                                                    ...
                                                                    if ($is_enable_animation) {
	                                                                    $addon_animation_class 	= isset( $addon['settings']['addon_animation']['name'] ) ? ' wow-wow ' . $addon['settings']['addon_animation']['name'] : '';
																...
																if ( $addon_type === 'addon' ) {
																	...
																	$addon_id = ! empty( $addon['settings']['wow_widget_id'] ) ? $addon['settings']['wow_widget_id'] : '';
																	$addon_class_for_widget = " wow-addon-{$addon_id}";
																}

																$base_name_id = '';
																if ( ! empty( $addon['settings']['wow_widget_id_base'] ) ) {
																	$base_name_id = $addon['settings']['wow_widget_id_base'];
																...
																echo "<div{$addon_css_id} class='wow-builder-addon wow-{$addon_type}-{$addon_id}{$addon_class_for_widget}{$addon_animation_class}{$addon_css_class}' {$addon_animation_data} data-addon-id='{$addon_id}'>";

                                                                echo "<div class='wow-{$addon_type}'>";
		...
		public function generate_inner_section($setting = array()){
			...
			if ( ! empty($setting['settings']['row_animation'])){
				...
				$animation_settings = (object) $setting['settings']['row_animation'];
				if ( ! empty($animation_settings->itemOpen) && $animation_settings->itemOpen){
					$animation_class = ' wow-wow wow-animated '.$animation_settings->name;
			...
			$row_css_class = isset($setting['settings']['row_class']) ? $setting['settings']['row_class']: '';
			?>
            <div <?php echo $row_css_id; ?> class="wow-row-parent wow-inner-row-parent wow-row-<?php echo $setting['id'].$animation_class; ?> <?php echo $row_css_class; ?>"<?php echo $animation_attr; ?>>
						...
						if ($columns_count){
							foreach ($setting['columns'] as $col){
								...
								if ( ! empty($col['settings']['col_animation']['itemOpen'])){
									...
									if ($enable_col_animation) {
										$col_css_class .= isset( $col_animation['name'] ) ? ' wow-wow wow-animated ' . $col_animation['name'] : '';
								...
								?>
                                <div <?php if ($col_css_id) echo 'id="'.$col_css_id.'"'; ?> class="wow-column-parent wow-column-parent-view wow-col-<?php echo $col['id']; ?> <?php echo $col_css_class; ?> " <?php echo $col_animation_data; ?>>
                                    <div class="wow-column">
                                        <div class="wow-builder-addons">
											<?php
											$addons_count = 0;
											...
											if ($addons_count){
												foreach ($col['addons'] as $addon){
                                                    ...
                                                    if ( isset($addon['visibility']) && $addon['visibility'] == true ) {
	                                                    ...
	                                                    $addon_animation_data = "";
	                                                    if (isset($addon['settings']['addon_animation'])){
		                                                    $addon_animation_class = isset($addon['settings']['addon_animation']['name']) ? ' wow-wow wow-animated ' . $addon['settings']['addon_animation']['name'] : '';
		                                                    $delay = isset($addon['settings']['addon_animation']['delay']) ? $addon['settings']['addon_animation']['delay'] : '';
		                                                    $duration = isset($addon['settings']['addon_animation']['duration']) ? $addon['settings']['addon_animation']['duration'] : '';
		                                                    $addon_animation_data = " data-wow-delay='{$delay}ms'  data-wow-duration='{$duration}ms' ";
	                                                    }
	                                                    ...
	                                                    echo "<div {$addon_css_id} class='wow-builder-addon wow-addon-{$addon_id} {$addon_css_class} {$addon_animation_class} ' {$addon_animation_data}  >";
		...
		public function bg_video( $video_settings = null ){
			...
			if ( $video_settings['videoType'] === 'local'){
				...
				return '<div class="wow-video-bg-wrap"><video class="wow-video-bg" autoPlay muted loop><source src="'.$video_settings['bgVideo']['url'].'" /></video></div>';
				...
				if (strpos( $video_settings['bgExternalVideo'] , 'yout') !== false) {
					return '<div class="wow-video-bg-wrap">'.str_replace( "?feature=oembed", "?feature=oembed&autoplay=1", $videoData ).'</div>';
				}
				if (strpos( $video_settings['bgExternalVideo'], 'vimeo') !== false) {
					return '<div class="wow-video-bg-wrap">'.str_replace( "?", "?autoplay=1&loop=1&", $videoData ).'</div>';
				}
				return '<div class="wow-video-bg-wrap">'.$videoData.'</div>';
			}
		}
	}
}