...
(function ($) {
  ...
  var nectarAdminStore = {
    ...
    bindEvents: function bindEvents() {
      ...
      $(window).on('mouseup', function () {
        ...
        $('.wpb_edit_form_elements .wpb_el_type_nectar_numerical').removeClass('scrubbing').removeClass('no-scrubbing');
      });
    },
    init: function init() {
      this.bindEvents();
  ...
  ConstrainedInput.prototype.getInitialSet = function () {
    var that = this;
    var classes = this.$el[0].className.split(/\s+/); // Store classname
  ...
  ConstrainedInput.prototype.trackActive = function () {
    ...
    this.$el.on('change', function () {
      // Store state.
      that.active = $(this).prop('checked'); // Alter icon.
  ...
  NectarNumericalInput.prototype.scrubbing = function () {
    ...
    this.$scrubber.on('mousedown', function () {
      ...
      that.$el.parents('.wpb_el_type_nectar_numerical').removeClass('no-scrubbing').addClass('scrubbing'); // Track that the mouse is down / store initial

      that.mouseDown = true;
      nectarAdminStore.mouseUp = false; // Starting pos

      that.initialX = nectarAdminStore.mouseX; // Empty
  ...
  NectarNumericalInput.prototype.scrubbingAlter = function (e) {
    ...
    this.calculatedVal = parseInt(this.currentVal) + parseInt(nectarAdminStore.mouseX - this.initialX) / 3; //// Who wants decimals??

    this.calculatedVal = Math.floor(this.calculatedVal); //// Stop number from going below 0

    if (this.zeroFloor && this.calculatedVal < 0) {
      this.$el.val(0);
    } else {
      this.$el.val(this.calculatedVal + this.unit); // Indicator
  ...
  function createDeviceGroup($selector) {
    ...
    var $title = $('body').find('.' + $selector).find('.group-title').clone(); // Group Markup.

    $('body').find('.' + $selector).wrapAll('<div class="' + $selector + '-wrap nectar-device-group-wrap vc_column" />'); // Header Markup.
  ...
  function deviceHighlightInUse($input) {
    ...
    var inUse = false; // Determine which icon is related
    ...
    $groupHeader.find('i[data-filter="' + iconSelector + '"]').removeClass('in-use'); // Check each input in the group for value set.
    //// Text inputs.
  ...
  function deviceGroupEvents() {
    $('.nectar-device-group-header i').on('click', function () {
      ...
      var group = $(this).parents('.nectar-device-group-header').next('.nectar-device-group-wrap'); // Already active.
      ...
      $(this).addClass('active'); // Display Grouping.

      group.find('> div').hide();
      group.find('> div.' + filter).fadeIn();
    });
    $('.nectar-device-group-header .device-selection i').each(function () {
      var $group = $(this).parents('.nectar-device-group-header').next('.nectar-device-group-wrap'); // On change.
  ...
  function colorOverlayImageUpdate() {
    ...
    if ($BGimage.find('img[src]').length > 0) {
      var src = $BGimage.find('img[src]').attr('src'); // full size preview.
  ...
  function colorOverlayPreview(el) {
    ...
    $colorPreview.insertAfter($('.col-md-6-last[data-vc-shortcode-param-name="color_overlay_2"]')); // Events.
  ...
  function colorOverlayPreviewUpdate() {
    setTimeout(function () {
      ...
      if ($useGrad.length > 0 && $useGrad.prop('checked') && $color1.length > 0 && $color2.length > 0 && $gradDir.length > 0) {
  ...
  function nectarFancyCheckboxes() {
    $('.vc_edit_form_elements .vc_shortcode-param:not(.constrain-icon) input[type="checkbox"].wpb_vc_param_value.checkbox').each(function () {
      ...
      $checkbox = $parent.find('input[type="checkbox"].wpb_vc_param_value.checkbox'); // Create HTML.
  ...
  function salientStudioSorting() {
    var $container = $(".vc_templates-list-default_templates"); // Create Markup.
    ...
    $('div[data-vc-ui-element="panel-templates"] .library-sorting').prepend('<label for="salient-studio-sorting">' + nectar_translations.sortby + '</label>'); // Events.

    $('body').on('change', 'select#salient-studio-sorting', function () {
      var $items = $(".vc_templates-list-default_templates > div"); // Convert Date to Standard JS Format.
  ...
  jQuery(document).ready(function ($) {
    ...
    var constrainedInputs = [],
        nectarNumericalInputs = []; // On modal open.

    $("#vc_ui-panel-edit-element").on('vcPanel.shown', function () {
      var $shortcode = $('#vc_ui-panel-edit-element[data-vc-shortcode]').length > 0 ? $('#vc_ui-panel-edit-element').attr('data-vc-shortcode') : ''; // Row.
      ...
      if ('vc_column' === $shortcode || 'vc_column_inner' === $shortcode || 'vc_row_inner' === $shortcode || 'vc_row' === $shortcode || 'image_with_animation' === $shortcode || 'divider' === $shortcode || 'fancy_box' === $shortcode) {
        deviceGroupEvents();
      } // Fancy checkboxes.


      nectarFancyCheckboxes(); // Video field.

      videoAttachFields(); // When full screen rows is active, do not create numerical inputs for disabled params
    ...
    salientElementSettingsLoading(); // Salient Studio Template Sorting

    salientStudioSorting(); // Fancy checkbox events.

    nectarFancyCheckboxEvents(); // Dynamic el styling - front end page builder
