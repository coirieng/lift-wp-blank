  ...
  $(selector).each(function () {
    ...
    el.on({
      click: function click(event) {
        ...
        if (!val) {
          _final = '[gallery ids="0"]';
        } else {
          _final = '[gallery ids="' + val + '"]';
        }

        var frame = wp.media.gallery.edit(_final);
        ...
        frame.state('gallery-edit').on('update', function (selection) {
          galleryInstance.find(".screenshot").html(""); //remove temp stylesheet that shows extra fields
