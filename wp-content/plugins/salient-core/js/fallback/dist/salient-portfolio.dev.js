...
(function ($) {
  ...
  SalientPortfolio.prototype.masonryInit = function () {
    ...
    this.$el.imagesLoaded(function () {
      ...
      instance.loadAnimationWaypoint(); // call the reLayout to get things rollin'
      ...
      $('.portfolio-loading').stop(true, true).fadeOut(200); // fadeIn items one by one
  ...
  SalientPortfolio.prototype.portfolioFiltersInit = function () {
    ...
    if (this.$el.parent().parent().find('.portfolio-filters-inline[data-alignment="left"]').length > 0 || this.$el.parent().parent().find('.portfolio-filters-inline[data-alignment="center"]').length > 0) {
  ...
  SalientPortfolio.prototype.isoClickFilter = function (e) {
    ...
    this.$el.isotope({
      filter: selector
    }).attr('data-current-cat', selector); // fade in all incase user hasn't scrolled down yet
  ...
  SalientPortfolio.prototype.reLayout = function () {
    ...
    this.portfolioItemWidths(); // sizing for large items

    if (!this.$el.is('[data-bypass-cropping="true"]')) {
      if (this.$el.find('.col.elastic-portfolio-item[class*="regular"]:visible').length > 0 || this.$el.find('.col.elastic-portfolio-item[class*="wide"]:visible').length > 0 || this.$el.find('.col.elastic-portfolio-item[class*="tall"]:visible').length > 0 || this.$el.find('.col.elastic-portfolio-item[class*="wide_tall"]:visible').length > 0) {
        var $gutterSize = this.$el.is('[data-gutter*="px"]') && this.$el.attr('data-gutter').length > 0 && this.$el.attr('data-gutter') != 'none' ? parseInt(this.$el.attr('data-gutter')) : 0;
        var multipler = window.innerWidth > 470 ? 2 : 1; // reset height for calcs
  ...
  SalientPortfolio.prototype.portfolioAccentColor = function () {
    this.$el.find('.col').each(function () {
      if ($(this).has('[data-project-color]')) {
        $(this).find('.work-info-bg, .bottom-meta').css('background-color', $(this).attr('data-project-color')); // style5
  ...
  SalientPortfolio.prototype.isotopeCatSelection = function () {
    ...
    itemCats = itemCats.split(' '); // remove the extra item on the end of blank space

    itemCats.pop(); // make sure the array has no duplicates

    itemCats = jQuery.unique(itemCats); // if user has chosen a set of filters to display - only show those
  ...
  SalientRecentProjectsFullScreen.prototype.sliderCalcs = function () {
    var $bodyBorderSize = $('.body-border-top').length > 0 && $(window).width() > 1000 ? $('.body-border-top').height() : 0; // frontend editor fix
  ...
  SalientRecentProjectsFullScreen.prototype.sliderResetRotate = function () {
    clearInterval(this.rotationInterval); // reinit autorotate
  ...
  SalientRecentProjectsFullScreen.prototype.sliderInit = function () {
    ...
    this.$el.find('.zoom-slider-controls .next').on('click', function () {
      ...
      $(this).parent().addClass('timeout'); // switch logic
    ...
    this.$el.find('.zoom-slider-controls .prev').on('click', function () {
      ...
      self.sliderResetRotate(self.$el.bind(self)); // switch logic
        ...
        swipe: {
          ...
          options: {
            ...
            tap: function tap(event, target) {
              if ($(target).attr('href') && !$(target).is('[target="_blank"]') && !$(target).is('[data-fancybox^="group_"]') && !$(target).is('[rel^="prettyPhoto"]') && !$(target).is('.magnific-popup') && !$(target).is('.magnific')) {
...
jQuery(document).ready(function ($) {
  "use strict"; // when not using Salient, get things rollin'
