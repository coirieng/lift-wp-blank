    ...
    $('.nectar-leaflet-map').each(function (i) {
      ...
      $('.map-marker-list.' + mapID).each(function () {
        $(this).find('.map-marker').each(function () {
          ...
          var markerLng = $(this).is('[data-lng]') && $(this).attr('data-lng').length > 0 ? parseFloat($(this).attr('data-lng')) : 0; //marker
          ////with infowindow
          ...
          if ($(this).attr('data-mapinfo') != '' && $(this).attr('data-mapinfo') != '<br />' && $(this).attr('data-mapinfo') != '<br/>') {
            if (!closePopupOnClickBool) {
              ...
              L.marker([markerLat, markerLng], iconObj).addTo(maps[j]).bindPopup($(this).attr('data-mapinfo'), {
                autoClose: false
              }).openPopup(); ////needed to avoid wrong center point (marker autopanning)    
