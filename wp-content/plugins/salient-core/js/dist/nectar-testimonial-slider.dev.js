...
(function ($) {
  ...
  NectarTestimonialSlider.prototype.createTestimonialControls = function () {
    ...
    if (this.type != 'multiple_visible' && this.type != 'multiple_visible_minimal') {
      ...
      if (this.el.find('blockquote').length > 1) {
        ...
        if (this.el.is('[data-style="minimal"]')) {
          // add next/prev
          this.el.append('<div class="testimonial-next-prev"><a href="#" class="prev fa fa-angle-left"></a><a href="#" class="next fa fa-angle-right"></a></div>'); // bind controls

          this.el.find('.testimonial-next-prev a').on('click', this.minimalNextPrevSelect); // start on first
          ...
          this.el.find('.controls').append('<span class="out-of">/</span><span class="total">' + this.el.find('blockquote').length + '</span>'); //// swipe for testimonials
        ...
        if (!this.el.is('[data-style="minimal"]')) {
          // bind controls
          this.el.find('.controls ul li').on('click', this.defaultPaginationSelect); // activate first slide

          this.el.find('.controls ul li').first().trigger('click'); // autorotate
    ...
    if (this.type == 'multiple_visible' || this.type == 'multiple_visible_minimal') {
      ...
      if (this.flickityEl.find('.vc_element.is-selected > blockquote').length > 0) {
        // starting
        this.flickityEl.find('.vc_element.is-selected > blockquote').addClass('is-selected'); // changed
  ...
  NectarTestimonialSlider.prototype.minimalNextPrevSelect = function (clicked) {
    ...
    if ($target.hasClass('next')) {
      ...
      currentHeight = $target.parents('.testimonial_slider').find('.slides ' + $frontEndEditorTestimonialDiv).eq($actualIndex).height(); // show slide
      ...
      currentHeight = $target.parents('.testimonial_slider').find('.slides ' + $frontEndEditorTestimonialDiv).eq($index - 1).height(); // show slide
  ...
  NectarTestimonialSlider.prototype.testimonialRotate = function () {
    ...
    var $currentTestimonial = this.el.find('.pagination-switch.active').parent().index(); // stop the rotation when toggles are closed
  ...
  NectarTestimonialSlider.prototype.testimonialSliderHeight = function () {
    if (this.el.is('.disable-height-animation') && !this.el.is('[data-style*="multiple_visible"]')) {
      ...
      this.el.find('.slides').css('height', $tallestQuote + 40 + 'px'); // show the slider once height is set
