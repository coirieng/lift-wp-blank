...
if ( ! class_exists('WOW_Ajax')){
	...
	class WOW_Ajax{
		...
		public function __construct() {
			...
			//Get Template from LIFT Page Builder Server
		...
		public function  wow_page_save(){
			...
			$success_array = array( 'msg' => __('LIFT Page Builder content has been updated.', 'wow-pagebuilder') );
			$error_array = array();

			$filename = "wow-page-css-{$page_id}.css";

			$upload_dir = wp_upload_dir();
			$dir = trailingslashit($upload_dir['basedir']) . 'lift-websitebuilder/';
		...
		/**
		 * @since 1.0.0
		 * Delete css file generated by LIFT Page Builder
		 */
		...
		public function wow_load_page_template(){
			if($this->switchAPI){
				...
				$upload_dir = wp_upload_dir();
				$dir = trailingslashit($upload_dir['basedir']) . 'lift-websitebuilder/cache/templates/';
		...
		public function wow_get_blocks(){
			...
			$upload_dir = wp_upload_dir();
			$dir = trailingslashit($upload_dir['basedir']) . 'lift-websitebuilder/cache/blocks/';
			$file_path_name = $dir . $cachedTemplateFile;
		...
		public function wow_clear_cache(){
			...
			$upload_dir     = wp_upload_dir();
			$dirPattern     = trailingslashit($upload_dir['basedir']).'lift-websitebuilder/cache/';
