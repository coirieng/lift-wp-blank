...
!function ($) {
  ...
  (Accordion = function Accordion($element, options) {
    ...
    var $this = this.$element,
        findSelector = function findSelector() {
    ...
    var that = this,
        findAnimationDuration = function findAnimationDuration() {
    ...
    that.isActive() || (that.isAnimated() ? (that.triggerEvent("beforeShow.vc.accordion"), $target.queue(function (next) {
      ...
      $target.addClass(that.animatingClass), $target.addClass(that.activeClass), ("object" == _typeof(opt) && opt.hasOwnProperty("changeHash") && opt.changeHash || void 0 === opt) && that.changeLocationHash(), that.triggerEvent("show.vc.accordion", opt), next();
    }).queue(function (next) {
