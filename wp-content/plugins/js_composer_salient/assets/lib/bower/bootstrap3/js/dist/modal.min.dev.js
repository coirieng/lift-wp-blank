...
!function ($) {
  ...
  function Plugin(option, _relatedTarget) {
    return this.each(function () {
      var $this = $(this),
          ...
          options = $.extend({}, Modal.DEFAULTS, $this.data(), "object" == _typeof(option) && option);
      data || $this.data("bs.modal", data = new Modal(this, options)), "string" == typeof option ? data[option](_relatedTarget) : options.show && data.show(_relatedTarget);
    });
  }

  Modal.VERSION = "3.1.1", Modal.DEFAULTS = {
    backdrop: !0,
    keyboard: !0,
    show: !0
    ...
    this.isShown && this.options.backdrop ? (callbackRemove = $.support.transition && animate, this.$backdrop = $('<div class="modal-backdrop ' + animate + '" />').appendTo(this.$body), this.$element.on("click.dismiss.bs.modal", $.proxy(function (e) {
      e.target === e.currentTarget && ("static" == this.options.backdrop ? this.$element[0].focus.call(this.$element[0]) : this.hide.call(this));
    }, this)), callbackRemove && this.$backdrop[0].offsetWidth, this.$backdrop.addClass("in"), callback && (callbackRemove ? this.$backdrop.one($.support.transition.end, callback).emulateTransitionEnd(150) : callback())) : !this.isShown && this.$backdrop ? (this.$backdrop.removeClass("in"), callbackRemove = function callbackRemove() {
      that.removeBackdrop(), callback && callback();
    }, $.support.transition && this.$element.hasClass("fade") ? this.$backdrop.one($.support.transition.end, callbackRemove).emulateTransitionEnd(150) : callbackRemove()) : callback && callback();
  ...
  $.fn.modal = Plugin, $.fn.modal.Constructor = Modal, $.fn.modal.noConflict = function () {
    ...
    var $this = $(this),
        option = $this.attr("href"),
        $target = $($this.attr("data-target") || option && option.replace(/.*(?=#[^\s]+$)/, "")),
        option = $target.data("bs.modal") ? "toggle" : $.extend({
      remote: !/#/.test(option) && option
    }, $target.data(), $this.data());
    $this.is("a") && e.preventDefault(), Plugin.call($target, option, this), $target.one("hide.bs.modal", function () {
