...
!function ($) {
  ...
  function Plugin(option) {
    return this.each(function () {
      var $this = $(this),
          ...
          options = $.extend({}, Carousel.DEFAULTS, $this.data(), "object" == _typeof(option) && option),
          action = "string" == typeof option ? option : options.slide;
      data || $this.data("bs.carousel", data = new Carousel(this, options)), "number" == typeof option ? data.to(option) : action ? data[action]() : options.interval && data.pause().cycle();
    });
  }

  Carousel.VERSION = "3.1.1", Carousel.DEFAULTS = {
    interval: 5e3,
    pause: "hover",
    wrap: !0
  ...
  }, Carousel.prototype.slide = function (type, relatedTarget) {
    var $active = this.$element.find(".item.active"),
        $next = relatedTarget || $active[type](),
        isCycling = this.interval,
        direction = "next" == type ? "left" : "right",
        slideEvent = "next" == type ? "first" : "last",
        that = this;

    if (!$next.length) {
      if (!this.options.wrap) return;
      $next = this.$element.find(".item")[slideEvent]();
    }

    if ($next.hasClass("active")) return this.sliding = !1;
    relatedTarget = $next[0], slideEvent = $.Event("slide.bs.carousel", {
      relatedTarget: relatedTarget,
      direction: direction
    });

    if (this.$element.trigger(slideEvent), !slideEvent.isDefaultPrevented()) {
      this.sliding = !0, isCycling && this.pause(), this.$indicators.length && (this.$indicators.find(".active").removeClass("active"), this.$element.one("slid.bs.carousel", function () {
      ...
      return $.support.transition && this.$element.hasClass("slide") ? ($next.addClass(type), $next[0].offsetWidth, $active.addClass(direction), $next.addClass(direction), $active.one($.support.transition.end, function () {
  ...
  $.fn.carousel = Plugin, $.fn.carousel.Constructor = Carousel, $.fn.carousel.noConflict = function () {
    ...
    var $this = $(this),
        $target = $($this.attr("data-target") || (slideIndex = $this.attr("href")) && slideIndex.replace(/.*(?=#[^\s]+$)/, "")),
        options = $.extend({}, $target.data(), $this.data()),
        slideIndex = $this.attr("data-slide-to");
    slideIndex && (options.interval = !1), Plugin.call($target, options), (slideIndex = $this.attr("data-slide-to")) && $target.data("bs.carousel").to(slideIndex), e.preventDefault();
