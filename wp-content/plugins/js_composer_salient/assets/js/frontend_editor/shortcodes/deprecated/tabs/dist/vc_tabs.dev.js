...
(function ($) {
  ...
  window.InlineShortcodeView_vc_tabs = window.InlineShortcodeView_vc_row.extend({
    ...
    buildTabs: function buildTabs(active_model) {
      if (active_model) {
    ...
    updateIfExistTab: function updateIfExistTab(model) {
      ...
      if ($tab.length) {
        $tab.attr('aria-controls', 'tab-' + model.getParam('tab_id')).find('a').attr('href', '#tab-' + model.getParam('tab_id')).text(model.getParam('title'));
        return true;
      }
    ...
    addElement: function addElement(e) {
      ...
      new window.vc.ShortcodesBuilder().create({
        shortcode: 'vc_tab',
        params: {
    ...
    removeTab: function removeTab(model) {
