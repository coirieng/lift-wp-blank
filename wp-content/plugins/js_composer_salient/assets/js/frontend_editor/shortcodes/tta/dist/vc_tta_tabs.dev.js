  ...
  window.InlineShortcodeView_vc_tta_tabs = window.InlineShortcodeView_vc_tta_accordion.extend({
    ...
    sectionUpdated: function sectionUpdated(model, justAppend) {
      ...
      if ($tabEl.length) {
        ...
        if (true !== justAppend) {
          models = _.pluck(_.sortBy(vc.shortcodes.where({
            parent_id: this.model.get('id')
          }), function (childModel) {
    ...
    getNextTab: function getNextTab($viewTab) {
    ...
    removeSection: function removeSection(modelId) {
      ...
      tabIsActive = $viewTab.hasClass(this.activeClass); // Make next tab active if needed
    ...
    updateSortingNavigation: function updateSortingNavigation() {
      var $tabs, self;
      self = this;
      $tabs = this.$el.find('.vc_tta-tabs-list'); // we are sorting a tabs navigation
    ...
    updateTabsPositions: function updateTabsPositions($panels) {
      var $tabs, $elements, tabSortableData;
      $tabs = this.$el.find('.vc_tta-tabs-list');

      if ($tabs.length) {
        $elements = [];
        tabSortableData = $panels.sortable('toArray', {
    ...
    updatePanelsPositions: function updatePanelsPositions($tabs) {
    ...
    renderSortingHelper: function renderSortingHelper(event, currentItem) {
    ...
    buildPagination: function buildPagination() {
      var params;
      this.removePagination(); // If tap-pos top append:
