...
(function ($) {
  ...
  window.InlineShortcodeView_vc_column = window.InlineShortcodeViewContainerWithParent.extend({
    ...
    render: function render() {
      ...
      this.prepend = false; // Here goes width logic

      $('<div class="vc_resize-bar"></div>').appendTo(this.$el).mousedown(this.startChangeSize);
    ...
    convertSize: function convertSize(width) {
      ...
      dev = !_.isUndefined(numbers[1]) && 0 <= _.indexOf(range, parseInt(numbers[1], 10)) ? parseInt(numbers[1], 10) : false; // Custom fix for 5 columns grid
