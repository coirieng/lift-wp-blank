  ...
  window.InlineShortcodeView_vc_row = window.InlineShortcodeView.extend({
    ...
    layoutEditor: function layoutEditor() {
      if (_.isUndefined(vc.row_layout_editor)) {
        vc.row_layout_editor = new vc.RowLayoutUIPanelFrontendEditor({
    ...
    addLayoutClass: function addLayoutClass() {
      ...
      this.layout = _.reject(vc.shortcodes.where({
        parent_id: this.model.get('id')
      }), function (model) {
    ...
    convertRowColumns: function convertRowColumns(layout, builder) {
      if (!layout) {
        return false;
      }
      ...
      vc.layout_old_columns = vc.shortcodes.where({
        parent_id: this.model.get('id')
      });
