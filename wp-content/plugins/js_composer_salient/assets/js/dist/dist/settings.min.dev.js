...
window.vc || (window.vc = {}), function () {
  ...
  $(function () {
    ...
    $('[data-vc-roles="part-state"]').on("click", function () {
      ...
      var value,
          $this = $(this),
          customSelector = $this.data("vcRolePart"),
          $customBlock = $('[data-vc-role-related-part="' + customSelector + '"]');
      if ("*" === $customBlock.data("vcRolePartState")) return !1;
      ...
      var $submitButton,
          data = {};
      e && e.preventDefault && e.preventDefault(), ($submitButton = $("#submit_btn")).attr("disabled", !0), $("#vc_wp-spinner").show(), data.action = $("#vc_settings-roles-action").val(), data.vc_nonce_field = $("#vc_nonce_field").val(), data.vc_roles = {}, $("[data-vc-role]").each(function () {
        var $this = $(this),
    ...
    var filterValue = $(this),
        $fieldset = filterValue.parents("fieldset").first(),
    ...
    var $this = $(this);
    ($this.is(":checked") ? $this.parents("tr:first").find('[data-vc-name!="' + $this.attr("data-vc-name") + '"]:checked') : $this.parents('[data-vc-roles="table"]').first().find('[data-vc-roles-select-all-checkbox="' + $this.data("vcCap") + '"]')).prop("checked", !1);
    ...
    $parent.find('[data-vc-cap="' + value + '"]:visible').prop("checked", checked), $relatedControl.prop("checked", checked), checked && _.defer(function () {
  ...
  _.isUndefined(window.less) || (window.vc.less = {}, window.vc.less.options = {
    ...
    return _.isEmpty(variablesData) || _.each(variablesData, function (value, key) {
      ...
      _.isString(value) ? (object = _.first(_.where(formData, {
        ...
        _.isUndefined(modifier.plain) || !_.isObject(modifier.plain) || _.isEmpty(modifier.plain) || _.each(modifier.plain, function (localValue) {
          localValue = localValue.replace("{{ value }}", objectValue);
...
window.i18nLocaleVcAutomapper = window.i18nLocaleSettings, function ($) {
  ...
  vc_am.vcGuid = function () {
    ...
    taggedString: function taggedString() {
  ...
  var message_timer,
      to_title,
      $vcSettings = $("#vc_settings-automapper");

  function show_message(text, $message) {
    message_timer && (window.clearTimeout(message_timer), $(".vc_settings-automapper").remove(), message_timer = !1);
    $message = $('<div class="vc_atm-message updated' + ($message ? " vc_message-" + $message : "") + '" style="display: none;"></div>');
  ...
  function showValidationError(text, $el) {
    !_.isUndefined($el) && $el.length || ($el = $(".tab_intro")), function (text, typeClass, timeout, remove) {
  ...
  to_title = function to_title(string) {
    return (string = string.replace(/_|-/, " ")).charAt(0).toUpperCase() + string.slice(1);
  };

  var ShortcodeModel = "?";
  -1 < window.ajaxurl.indexOf("?") && (ShortcodeModel = "&"), request_url = window.ajaxurl + ShortcodeModel + "vc_action=automapper", AppView = function AppView(method, model, options) {
    var data = "create" === method ? (model.set("id", vc_am.vcGuid()), {
      vc_action: "create",
    ...
    $.ajax({
      ...
      result.success && ((result = (result = result.data) && "read" === method ? result : model) ? "read" === method && options.success(result) : options.error("Not found"));
    }).error(function (data) {});
  };
  var ShortcodeModel = Backbone.Model.extend({
    ...
    sync: AppView
      ...
      AppView = Backbone.Collection.extend({
    model: ShortcodeModel,
    sync: AppView
  });
  vc_am.shortcodes = new AppView();
  ...
  var EditFormView,
    ...
    removeEditForm: function removeEditForm() {
    ...
    parseShortcode: function parseShortcode(data) {
      data && data.preventDefault && data.preventDefault();
      var params = [],
          attr = $("#vc_atm-shortcode-string").val();
      return !_.isEmpty(attr) && (data = attr.match(this.shortcodesRegexp())) ? (attr = wp.shortcode.attrs(data[3]), _.each(attr.named, function (value, key) {
        params.push({
          param_name: key,
          type: "textfield",
      ...
      EditFormInnerView = (EditFormView = AppView.extend({
    ...
    changeParamType: function changeParamType($parent) {
      var $this = $($parent.currentTarget),
          $parent = $this.parents(".vc_fields");
      "hidden" === $this.val() ? ($parent.find('[name="heading"]').attr("disabled", !0), $parent.find('[name="description"]').attr("disabled", !0)) : ($parent.find('[name="heading"]').attr("disabled", !1), $parent.find('[name="description"]').attr("disabled", !1));
    },
    setTagFieldActive: function setTagFieldActive(e) {
    ...
    setParamFieldActive: function setParamFieldActive(index) {
      index = $(index.currentTarget).parents(".vc_param:first").index();
    ...
    setContentParamFieldActive: function setContentParamFieldActive(e) {
    ...
    save: function save(data) {
      data && data.preventDefault && data.preventDefault(), this.$el.find(".vc_error").removeClass("vc_error");
      data = {
        tag: $("#vc_atm-tag").val(),
    ...
    setContentParam: function setContentParam(e) {
    ...
    addAllParams: function addAllParams() {
    ...
    removeParamField: function removeParamField(name) {
      $('.vc_param-name[value="' + name + '"]').parents(".vc_param").remove();
    },
    addParamField: function addParamField(attr) {
    ...
    setParamSorting: function setParamSorting() {
    ...
    deleteParam: function deleteParam(e) {
    ...
    formSubmit: function formSubmit(node) {
      node && node.preventDefault && node.preventDefault(), _.isObject(node) && this.addFormView && !_.isEmpty(node.currentTarget) && !_.isEmpty(node.currentTarget[0]) && (node = node.currentTarget[0], $(node).is("#vc_atm-shortcode-string") && this.addFormView.parseShortcode());
    },
    ...
    addOne: function addOne(view) {
      view = new ComplexShortcodeView({
        model: view
      });
  ...
  function vc_setCookie(c_name, value, exdays) {
    var c_value = new Date();
    c_value.setDate(c_value.getDate() + exdays);
    c_value = encodeURIComponent(value) + (null === exdays ? "" : "; expires=" + c_value.toUTCString());
    document.cookie = c_name + "=" + c_value;
  }

  function vc_getCookie(c_name) {
    for (var x, y, ARRcookies = document.cookie.split(";"), i = 0; i < ARRcookies.length; i++) {
  ...
  $(".wpb_settings_accordion").accordion({
    ...
    return !0;
  }), $(".vc_show_example").on("click", function ($helper) {
  ...
  $("#vc_settings-color").on("submit", function (e) {
    var modifyVars, variablesDataLinker, $spinner, $submitButton;
    e.preventDefault(), lessBuilding || ($submitButton = $("#submit_btn"), $("#wpb_js_use_custom").prop("checked") && "restore_color" !== $("#vc_settings-color-action").val() ? (lessBuilding = !0, modifyVars = $(this).serializeArray(), variablesDataLinker = $submitButton.data("vc-less-variables"), $spinner = $('<span class="vc_settings-spinner vc_ui-wp-spinner"></span>'), $submitButton.val(window.i18nLocaleSettings.saving), $spinner.insertBefore($submitButton).show(), _.delay(function () {
      vc.less.build({
        ...
        _.isUndefined($form) || _.isUndefined($form.css) ? _.isUndefined(error) || (window.console && window.console.warn && window.console.warn("build error", error), showMessageMore(window.i18nLocaleSettings.save_error + ". " + error, "error", void 0, !0).insertBefore($submitButton.parent()).fadeIn(500), $submitButton.val(window.i18nLocaleSettings.save), lessBuilding = !1, $spinner.remove()) : ($('[name="wpb_js_compiled_js_composer_less"]').val($form.css), $form = $("#vc_settings-color"), $.ajax({
          type: "POST",
          url: $form.attr("action"),
