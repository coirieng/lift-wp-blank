...
window.vc_iframe = {
  ...
  window.vc_iframe.showNoContent = function (show) {
    ...
    $('.container-wrap').css('pointer-events', 'auto'); //to fix full width content etc

    $(window).trigger('smartresize');
    vc_iframe.reload(); //vc col mobile fixes
    ...
    var setSectionSortable,
        ...
        $rowSortable,
        $elementsSortable,
        _this = window.vc_iframe;
    parent.vc.$page.addClass("vc-main-sortable-container");
    var $sectionSortables,
        $main = $(parent.vc.$page); //nectar addition - full page row ordering

    var FPstartingIndex = 0,
        FPendingIndex = 0;
    setTimeout(function () {
      $('#fp-nav ul').sortable({
        ...
        stop: function stop(event, ui) {
          ...
          FPendingIndex = ui.item.index(); //move the rows based on collected indexes
          ...
          vc_iframe.reload(); // used to reinit sortable
    ...
    }), setElementsSortable = function setElementsSortable() {
    ...
    }, setRowSortable = function setRowSortable() {
    ...
    }, (setSectionSortable = function setSectionSortable() {
    ...
    for (var i = 0; this.inline_scripts[i];) {
      $(this.inline_scripts[i]).insertAfter(".js_placeholder_" + i), $(".js_placeholder_" + i).remove(), i++;
    }

    this.inline_scripts = [];
  }, window.vc_iframe.loadInlineScriptsBody = function () {
    for (var i = 0; this.inline_scripts_body[i];) {
      $(this.inline_scripts_body[i]).insertAfter(".js_placeholder_inline_" + i), $(".js_placeholder_inline_" + i).remove(), i++;
    }
    ...
    if (src.match(/load\-scripts\.php/)) {
      for (i in (scripts_string = src.match(/load%5B%5D=([^&]+)/)[1]) && (scripts = scripts_string.split(",")), scripts) {
        ls_rc = "load-script:" + scripts[i], vc_iframe.loaded_script[window.parent.vc_globalHashCode(ls_rc)] || (window.vc_iframe.loaded_script[window.parent.vc_globalHashCode(ls_rc)] = ls_rc, scripts_to_add.push(scripts[i]));
      }
    ...
    }), this.collectScriptsData(), this.loadInlineScripts(), this.loadInlineScriptsBody(), this.activities_list) {
    ...
    vc_iframe.scripts_to_wait && vc_iframe.scripts_to_load ? (window.vc_iframe.scripts_to_load.each(function () {
      ...
      if (window.vc_iframe.reload_safety_call = !0, $element.is("script")) {
        ...
        }) : (--window.vc_iframe.scripts_to_wait, vc_iframe.scripts_to_wait < 1 && window.vc_iframe.reload());else {
          try {
    ...
    if (view.$accordion.hasClass('toggles')) {
      ...
      $(view.$accordion.get(0)).sortable({
        ...
        placeholder: "vc_placeholder-accord",
        start: function start(event, ui) {
          ui.helper.parent().addClass('nectar-sortable-block-display'); //store width before changing

          widthStorage = ui.helper.parent().find('> *:first-child').outerWidth(); //set widths equal to child length
        ...
        stop: function stop(event, ui) {
          ...
          ui.item.parent().find('> *').css('width', widthStorage + 'px'); //needed to reset order in flickity
