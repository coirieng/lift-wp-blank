...
_.isUndefined(window.vc) && (window.vc = {
  ...
  var VcColumnOffsetParam = Backbone.View.extend({
    ...
    setLgPlaceholders: function setLgPlaceholders() {
  ...
  media = wp.media, preloader_url = ajaxurl.replace(/admin\-ajax\.php/, "images/wpspin_light.gif"), media.controller.VcCssSingleImage = media.controller.VcSingleImage.extend({
    setCssEditor: function setCssEditor(view) {
        ...
        ids = [], $(".media-modal").addClass("processing-media"), selection.each(function (model) {
          ...
          $.ajax({
            ...
            if ("function" == typeof callback) {
              for (attachments = [], promises = [], i = 0; i < response.data.ids.length; i++) {
                attachment = wp.media.model.Attachment.get(response.data.ids[i]), promises.push(attachment.fetch()), attachments.push(attachment);
              }
    ...
    setCurrentBgImage: function setCurrentBgImage(value) {
      var url,
          template,
          image_split = /([^\?]+)(\?id=\d+){0,1}/,
          id = "";
    ...
    changeSimplify: function changeSimplify() {
      _.debounce(this.setSimplify, 100)();
    },
    setSimplify: function setSimplify() {
      this.simplifiedMode(this.$simplify[0].checked);
    },
    simplifiedMode: function simplifiedMode(enable) {
    ...
    parseAtts: function parseAtts(string) {
    ...
    getBackgroundImageSrc: function getBackgroundImageSrc() {
    ...
    getFields: function getFields(type) {
      var data = [];
    ...
    getSimplifiedField: function getSimplifiedField(type) {
