...
!function () {
  ...
  vc.gridItemEditor = !0, vc.Storage.prototype.getContent = function () {
  ...
  }, vc.visualComposerView.prototype.openVcGitemEditForm = function (vcGitemModel) {
    vcGitemModel && vcGitemModel.preventDefaul && vcGitemModel.preventDefault();
    vcGitemModel = vc.shortcodes.findWhere({
      shortcode: "vc_gitem"
    });
  ...
  }, vc.visualComposerView.prototype.changeAnimation = function (params) {
    var $dropdown,
        model = vc.shortcodes.findWhere({
      shortcode: "vc_gitem_animated_block"
    });
    if (!model) return !1;
    $dropdown = $(params.currentTarget), (params = $.extend({}, model.get("params"))).animation = $dropdown.val(), model.save({
      params: params
    ...
    var params = this.model.get("params"),
        merged_params = vc.getMergedParams(this.model.get("shortcode"), _.extend({}, vc.getDefaults(this.model.get("shortcode")), params));
    return _.isUndefined(params.content) || (merged_params.content = params.content), {
      action: "vc_edit_form",
    ...
    setCEmpty: function setCEmpty() {
      this.$content.addClass("vc_empty-c");
    },
    unsetCEmpty: function unsetCEmpty() {
    ...
    addAnimatedBlock: function addAnimatedBlock(animatedBlock) {
      animatedBlock && animatedBlock.preventDefault && animatedBlock.preventDefault(), this.animatedBlock || (vc.storage.lock(), animatedBlock = vc.shortcodes.create({
        shortcode: "vc_gitem_animated_block",
    ...
    addCZone: function addCZone(model) {
      var $column = $(model.currentTarget),
          model = $column.data("vcPosition");
      if (this.cZone) return this.updateCZonePosition($column, model), !1;
      (model = new vc.shortcode({
        shortcode: "vc_gitem_add_c_zone",
        params: {
          cZonePosition: this.getCZoneModelOrder(model),
          position: model
        },
        parent_id: this.model.get("id")
      })).view = this, vc.add_element_block_view.render(model);
    },
    updateCZonePosition: function updateCZonePosition($column, position) {
      this.setCZonePosition(position), this.setCZoneOrder(position), this.changeCZonePosition($column), this.setCssPosition($column);
    },
    setCZoneOrder: function setCZoneOrder(position) {
      this.cZone.save({
        order: this.getCZoneModelOrder(position)
      });
    },
    changeCZonePosition: function changeCZonePosition($column) {
      this.cZone.view.$el.appendTo($column);
    },
    setCZonePosition: function setCZonePosition(position) {
      this.model.save({
    ...
    setCZoneHeight: function setCZoneHeight() {
      var $left = $('[data-vc-gitem="add-c"][data-vc-position="left"]'),
          max_height = $('[data-element_type="vc_gitem_animated_block"]'),
          $right = $('[data-vc-gitem="add-c"][data-vc-position="right"]');
      $left.height("auto"), $right.height("auto"), max_height = Math.max($left.height(), max_height.height(), $right.height()), $left.css("height", max_height), $right.css("height", max_height);
    },
    getCZoneModelOrder: function getCZoneModelOrder(position) {
    ...
    startCDragging: function startCDragging(event, ui) {
      this.$el.addClass("vc_sorting");
    },
    stopCDragging: function stopCDragging(event, ui) {
      this.$el.removeClass("vc_sorting");
    },
    savePositionC: function savePositionC(event, position) {
      var cBlockContainer = position.item.parent(),
          position = cBlockContainer.data("vcPosition") || "bottom";
      vc.storage.lock(), this.cZone && this.cZone.save({
        order: this.getCZoneModelOrder(position)
      }), this.setCZonePosition(position), this.setCssPosition(cBlockContainer);
    },
    setCssPosition: function setCssPosition($container) {
    ...
    setAnimationClassToBlock: function setAnimationClassToBlock() {
    ...
    setAnimationDropDown: function setAnimationDropDown() {
    ...
    addAZone: function addAZone(e) {
      e && e.preventDefault && e.preventDefault(), this.addZone("a");
    },
    addBZone: function addBZone(e) {
    ...
    addTab: function addTab(zone, $zone) {
      $zone = $('<li class="vc_gitem-animated-block-tab vc_gitem-tab-' + zone + '"><a href="#" data-vc-gitem-tab-link="' + zone + '"><span class="vc_label">' + this.getZoneLabel(zone) + '</span></a><a class="vc_control column_edit" data-vc-control="edit" data-vc-zone-model-id="' + $zone + '"><i class="vc-composer-icon vc-c-icon-mode_edit"></i></a></li>');
      $zone.appendTo('[data-vc-gitem-animated-block="tabs"]'), "a" === zone && this.switchZone($zone.find("a"));
    },
    getZoneLabel: function getZoneLabel(zone) {
      var zoneSettings = vc.map["vc_gitem_zone_" + zone] || !1;
      return zoneSettings && zoneSettings.name ? zoneSettings.name : zone;
    },
    activateZone: function activateZone(e) {
      var $control = $(e.currentTarget);
      e && e.preventDefault && e.preventDefault(), $control.hasClass("vc_active") || this.switchZone($control);
    },
    showEditForm: function showEditForm(model_id) {
      model_id && model_id.preventDefault && model_id.preventDefault(), (model_id = $(model_id.currentTarget).data("vcZoneModelId")) && vc.app.views[model_id] && vc.app.views[model_id].editElement();
    },
    ...
    setAEmpty: function setAEmpty() {
      this.$content.addClass("vc_empty-a");
    },
    unsetAEmpty: function unsetAEmpty() {
      this.$content.removeClass("vc_empty-a");
    },
    setBEmpty: function setBEmpty() {
      this.$content.addClass("vc_empty-b");
    },
    unsetBEmpty: function unsetBEmpty() {
    ...
    addElement: function addElement(row) {
      row && row.preventDefault && row.preventDefault(), vc.storage.lock(), row = vc.shortcodes.create({
    ...
    changeTextAlign: function changeTextAlign($control) {
      if ($control) {
        if ($control.preventDefault && $control.preventDefault(), ($control = $($control.currentTarget)).hasClass("vc_active")) return !1;
    ...
    setTextAlign: function setTextAlign(params) {
      params = _.extend({}, this.model.get("params"), {
        align: params
      });
      this.model.save({
  ...
  }), vc.AddElementUIPanelBackendEditor.prototype.createElement = function (cZoneRow) {
    cZoneRow && cZoneRow.preventDefault && cZoneRow.preventDefault();
    var cZone,
        model = !1,
        showSettings = $(cZoneRow.currentTarget).data("tag");
    !1 !== this.model && "vc_gitem_add_c_zone" === this.model.get("shortcode") ? (this.model.view.setCZonePosition(this.model.getParam("position")), vc.storage.lock(), cZone = vc.shortcodes.create({
      shortcode: "vc_gitem_zone_c",
    ...
    }), vc.storage.lock(), model = vc.shortcodes.create({
      shortcode: "vc_gitem_col",
      params: _.extend({
        width: "1/1"
      }, vc.getDefaults("vc_gitem_col")),
      parent_id: cZoneRow.get("id")
    }).get("id"), cZone = null) : !1 !== this.model && (model = this.model.get("id")), model = vc.shortcodes.create({
      shortcode: showSettings,
      parent_id: model,
      params: vc.getDefaults(showSettings),
      root_id: vc.shortcodes.findWhere({
        shortcode: "vc_gitem"
      }) || !1
    }), "vc_gitem_row" === showSettings && vc.shortcodes.create({
      shortcode: "vc_gitem_col",
      params: {
        width: "1/1"
      },
    ...
    }), this.model = model, showSettings = !(_.isBoolean(vc.getMapped(showSettings).show_settings_on_create) && !1 === vc.getMapped(showSettings).show_settings_on_create), this.model.get("shortcode"), this.hide(), showSettings && this.showEditForm();
