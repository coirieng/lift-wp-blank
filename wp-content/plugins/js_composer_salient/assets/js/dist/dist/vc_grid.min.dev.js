...
!function ($) {
  ...
  var vcGridStyleAll = function vcGridStyleAll(grid) {
  ...
  var vcGridStyleLoadMore = function vcGridStyleLoadMore(grid) {
  ...
  !function ($) {
  ...
  }(window.jQuery), (vcGridStyleLazy = function vcGridStyleLazy(grid) {
    ...
    var self, fn;
    this.$content.find('.vc_grid_filter-item:not(".vc_visible-item")').length && (self = this, fn = function fn() {
  ...
  var vcGridStylePagination = function vcGridStylePagination(grid) {
  ...
  vcGridStylePagination.prototype.init = function () {
    ...
    var that, $vcCarousel;
    $.fn.vcOwlCarousel && (($vcCarousel = (that = this).$content.data("owl.vccarousel")) && $vcCarousel.destroy(), this.$content.on("initialized.owl.vccarousel", function (itemsCount) {
      ...
      }), -1 < that.settings.paging_design.indexOf("pagination") && (itemsCount = $carousel.items().length, ($pagination = $("<div></div>").addClass("vc_grid-pagination").appendTo(that.$el)).twbsPagination({
  ...
  var vcGridStyleAllMasonry = function vcGridStyleAllMasonry(grid) {
  ...
  vcGridStyleAllMasonry.prototype = _.extend({}, window.vcGridStyleAll.prototype, {
    ...
    filter: function filter(_filter) {
      if (_filter = _.isUndefined(_filter) || "*" === _filter ? "" : _filter, this.filterValue == _filter) return !1;
      this.filterValue = _filter, this.$content.data("masonry") && this.$content.masonry("destroy"), this.masonryEnabled = !1, this.$content.find(".vc_visible-item").removeClass("vc_visible-item"), this.$content.find(".vc_grid-item" + this.filterValue), this.filtered = !0, $(window).resize(this.setMasonry), this.setMasonry(), this.showItems();
    },
    setIsLoading: function setIsLoading() {
      this.$el.append(this.$loader), this.isLoading = !0;
    },
    unsetIsLoading: function unsetIsLoading() {
      this.isLoading = !1, this.$loader && this.$loader.remove();
    },
    setItems: function setItems(els) {
      this.masonryEnabled ? this.$content.masonry("appended", els) : this.setMasonry();
    },
    setMasonry: function setMasonry() {
  ...
  var vcGridStyleLazyMasonry = function vcGridStyleLazyMasonry(grid) {
  ...
  vcGridStyleLazyMasonry.prototype = _.extend({}, window.vcGridStyleLazy.prototype, {
    ...
    filter: function filter(_filter2) {
      if (_filter2 = _.isUndefined(_filter2) || "*" === _filter2 ? "" : _filter2, this.filterValue == _filter2) return !1;
      this.$content.data("masonry") && this.$content.masonry("destroy"), this.masonryEnabled = !1, this.$content.find(".vc_visible-item, .vc_grid_filter-item").removeClass("vc_visible-item vc_grid_filter-item"), this.filterValue = _filter2, this.$content.find(".vc_grid-item" + this.filterValue).addClass("vc_grid_filter-item"), this.filtered = !0, $(window).resize(this.setMasonry), this.setMasonry(), _.defer(this.showItems);
    },
    setItems: function setItems(els) {
      this.masonryEnabled ? this.$content.masonry("appended", els) : this.setMasonry();
    },
  ...
  var vcGridStyleLoadMoreMasonry = function vcGridStyleLoadMoreMasonry(grid) {
  ...
  vcGridStyleLoadMoreMasonry.prototype = _.extend({}, window.vcGridStyleLoadMore.prototype, {
    ...
    filter: function filter(_filter3) {
      if (_filter3 = _.isUndefined(_filter3) || "*" === _filter3 ? "" : _filter3, this.filterValue == _filter3) return !1;
      this.$content.data("masonry") && this.$content.masonry("destroy"), this.masonryEnabled = !1, this.$content.find(".vc_visible-item, .vc_grid_filter-item").removeClass("vc_visible-item vc_grid_filter-item"), this.filterValue = _filter3, this.$content.find(".vc_grid-item" + this.filterValue).addClass("vc_grid_filter-item"), this.filtered = !0, $(window).resize(this.setMasonry), this.setMasonry(), this.showItems();
    },
    setIsLoading: function setIsLoading() {
  ...
  var VcGrid,
    ...
    itemAnimationDelay: [],
    clearAnimationDelays: function clearAnimationDelays() {
      _.each(this.itemAnimationDelay, function (id) {
        window.clearTimeout(id);
      }), this.itemAnimationDelay = [];
    }
  };
  (VcGrid = function VcGrid(el) {
    ...
    window.vc_iframe ? (this.$el.off("click.zonePostLink").on("click.zonePostLink", "[data-vc-link]", function () {
      ...
      var httpRequest,
          ...
          timeout,
          $this = $(this);
      $this.hasClass("vc-gitem-link-ajax") && (e.preventDefault(), httpRequest = new XMLHttpRequest(), $this.addClass("vc-spinner"), httpRequest.open("GET", $this.attr("href"), !0), httpRequest.send(), removeSpinner = function removeSpinner() {
    ...
    var $control;
    if (filter && filter.preventDefault && filter.preventDefault(), this.style && $.camelCase("filter-" + this.style), filter = ($control = $(filter.currentTarget).find("[data-vc-grid-filter-value]")).data("vcGridFilterValue"), $control.hasClass("vc_active")) return !1;
    ...
    _.isUndefined(requestData) && (requestData = {}), requestData = _.extend({
