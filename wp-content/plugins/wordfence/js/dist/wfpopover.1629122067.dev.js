...
+function ($) {
  ...
  var WFTooltip = function WFTooltip(element, options) {
    this.type = null;
    this.options = null;
    this.enabled = null;
    this.timeout = null;
  ...
  WFTooltip.prototype.show = function () {
    ...
    if (this.hasContent() && this.enabled) {
      ...
      if (autoPlace) {
        ...
        placement = placement == 'wf-bottom' && pos.bottom + actualHeight > viewportDim.bottom ? 'wf-top' : placement == 'wf-top' && pos.top - actualHeight < viewportDim.top ? 'wf-bottom' : placement == 'wf-right' && pos.right + actualWidth > viewportDim.width ? 'wf-left' : placement == 'wf-left' && pos.left - actualWidth < viewportDim.left ? 'wf-right' : placement;
      ...
      $.support.transition && this.$tip.hasClass('wf-fade') ? $tip.one('bsTransitionEnd', complete).emulateTransitionEnd(WFTooltip.TRANSITION_DURATION) : complete();
    }
  };
  ...
  WFTooltip.prototype.applyPlacement = function (offset, placement) {
    ...
    $tip.addClass('wf-in'); // check to see if placing tip in new offset caused the tip to resize itself
  ...
  WFTooltip.prototype.hide = function (callback) {
    ...
    $tip.removeClass('in');
    $.support.transition && $tip.hasClass('wf-fade') ? $tip.one('bsTransitionEnd', complete).emulateTransitionEnd(WFTooltip.TRANSITION_DURATION) : complete();
    this.hoverState = null;
    return this;
  };
  ...
  $.fn.wftooltip = Plugin;
  $.fn.wftooltip.Constructor = WFTooltip; // TOOLTIP NO CONFLICT
  // ===================
  ...
  var WFPopover = function WFPopover(element, options) {
  ...
  WFPopover.prototype.setContent = function () {
    ...
    $tip.removeClass('wf-fade wf-top wf-bottom wf-left wf-right wf-in'); // IE8 doesn't accept hiding via the `:empty` pseudo selector, we have to do
  ...
  $.fn.wfpopover = Plugin;
  $.fn.wfpopover.Constructor = WFPopover; // POPOVER NO CONFLICT
  // ===================
