...
(function ($) {
  ...
  LiveTrafficViewModel = function LiveTrafficViewModel(listings, filters) {
    var self = this;
    var listingIDTable = {};
    ...
    var getURLEncodedFilters = function getURLEncodedFilters() {
      var dataString = '';
    ...
    var pullDownListings = function pullDownListings(options, callback) {
  ...
  LiveTrafficViewModel.truncateText = function (text, maxLength) {
    ...
    if (text && text.length > maxLength) {
      return text.substring(0, Math.round(maxLength)) + "\u2026"; // return text.substring(0, Math.round(maxLength / 2)) + " ... " + text.substring(text.length - Math.round(maxLength / 2));
    }

    return text;
  };

  var ListingModel = function ListingModel(data, groupBy) {
    var self = this;
    ...
    self.username = ko.observable(''); // New fields/columns
    ...
    }, self); // Use the same format as these update.
      ...
      switch (self.action()) {
        ...
        case 'blocked:waf':
          ...
          if (_typeof(data) === 'object') {
            ...
            var paramValue = WFAD.base64_decode(data.paramValue); // var category = data.category;
    ...
    self.showWhoisOverlay = function () {
      ...
      WFAD.ajax('wordfence_whois', {
        ...
        overlayBody.stop().animate({
          opacity: 1
        }, 200).html('<h4 style=\'margin-top:0;\'>' + __('WHOIS LOOKUP') + '</h4>' + whoisHTML);
    ...
    self.showRecentTraffic = function () {
      ...
      WFAD.ajax('wordfence_recentTraffic', {
        ...
        overlayBody.stop().animate({
          opacity: 1
        }, 200).html('<h3 style=\'margin-top:0;\'>' + __('Recent Activity') + '</h3>' + result.result);
        $(window).trigger('wf-live-traffic-overlay-bind', self);
        WFAD.avatarLookup();
      });
    };
  ...
  var ListingsFilterModel = function ListingsFilterModel(viewModel, param, value, operator) {
    var self = this;
    ...
    self.filterOperatorOptions = ko.observableArray([equalsOperator, notEqualsOperator, containsOperator, matchOperator]);
    self.filterParamOptions = ko.observableArray([new FilterParamModel('type', __('Type'), 'enum', [new FilterParamEnumOptionModel('human', __('Human')), new FilterParamEnumOptionModel('bot', __('Bot'))]), new FilterParamModel('user_login', __('Username')), new FilterParamModel('userID', __('User ID')), new FilterParamModel('isGoogle', __('Google Bot'), 'bool'), new FilterParamModel('ip', __('IP')), new FilterParamModel('ua', __('User Agent')), new FilterParamModel('referer', __('Referer')), new FilterParamModel('url', __('URL')), new FilterParamModel('statusCode', __('HTTP Response Code')), new FilterParamModel('action', __('Firewall Response'), 'enum', [new FilterParamEnumOptionModel('', __('OK')), new FilterParamEnumOptionModel('throttled', __('Throttled')), new FilterParamEnumOptionModel('lockedOut', __('Locked Out')), new FilterParamEnumOptionModel('blocked', __('Blocked'), containsOperator), new FilterParamEnumOptionModel('blocked:waf', __('Blocked WAF'))]), new FilterParamModel('action', __('Logins'), 'enum', [new FilterParamEnumOptionModel('loginOK', __('Logged In')), new FilterParamEnumOptionModel('loginFail', __('Failed Login')), new FilterParamEnumOptionModel('loginFailInvalidUsername', __('Failed Login: Invalid Username')), new FilterParamEnumOptionModel('loginFailValidUsername', __('Failed Login: Valid Username'))]), new FilterParamModel('action', __('Security Event'))]);
    ...
    self.urlEncoded = function () {
      ...
      return 'param[]=' + encodeURIComponent(self.param()) + '&value[]=' + encodeURIComponent(value) + '&operator[]=' + encodeURIComponent(self.operator());
    };
  };

  var PresetFilterModel = function PresetFilterModel(text, value, filters, groupBy) {
  ...
  var FilterParamModel = function FilterParamModel(param, text, type, values) {
    this.text = ko.observable('');
  ...
  var FilterParamEnumOptionModel = function FilterParamEnumOptionModel(value, label, operator) {
    this.value = ko.observable('');
  ...
  var FilterOperatorModel = function FilterOperatorModel(operator, text) {
    this.text = ko.observable('');
    this.operator = ko.observable('');
    this.text(text);
  ...
  var GroupByModel = function GroupByModel(param, text) {
    this.text = ko.observable('');
    this.param = ko.observable('');
    this.text(text);
    this.param(param);
  ...
  ko.bindingHandlers.datetimepicker = {
    init: function init(element, valueAccessor, allBindingsAccessor) {
      ...
      var options = allBindingsAccessor().datepickerOptions || {},
          $el = $(element);
      $el.datetimepicker(options); //handle the field changing by registering datepicker's changeDate event
  ...
  $(function () {
    ...
    $('#wf-lt-preset-filters').wfselect2({
      templateSelection: function templateSelection(value) {
    ...
    $(window).on('scroll', function () {
      ...
      if (firstLTRow.length > 0 && firstLTRow.offset().top + firstLTRow.height() < win.scrollTop() + adminBar.outerHeight() + 20 || groupedListings.filter(':visible').length > 0) {
      ...
      if (hasScrolled && !loadingListings && currentScrollBottom >= scrollThreshold) {
        ...
        hasScrolled = true; // console.log('no infinite scroll');
      }
