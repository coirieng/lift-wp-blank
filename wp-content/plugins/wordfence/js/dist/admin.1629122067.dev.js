...
(function ($) {
  ...
  if (!window['wordfenceAdmin']) {
    //To compile for checking: java -jar /usr/local/bin/closure.jar --js=admin.js --js_output_file=test.js
    window['wordfenceAdmin'] = {
      ...
      newestActivityTime: 0,
      //must be 0 to force loading of all initially
      ...
      init: function init() {
        ...
        $('#doSendEmail').click(function () {
          ...
          WFAD.ajax('wordfence_sendDiagnostic', {
            email: $('#_email').val(),
            ticket: ticket
          }, function (res) {
            if (res.result) {
        ...
        if ($('.wf-options-controls-spacer').length) {
          //The WP code doesn't move update nags and we need to
        ...
        }).triggerHandler('change'); //Text field option
        ...
        if ($.fn.wfselect2) {
          $('.wf-option.wf-option-token select').wfselect2({
            ...
            selected.each(function (index, value) {
              ...
              li.children('a.wf-destroy-tag-selected').off('click.wfselect2-copy').on('click.wfselect2-copy', function (e) {
        ...
        $(document).focus(); // (docs|support).wordfence.com GA links
      ...
      sectionInit: function sectionInit() {
        var self = this;
        this.mode = false;
      ...
      sendTestEmail: function sendTestEmail(email) {
        var self = this;
        this.ajax('wordfence_sendTestEmail', {
          email: email
        }, function (res) {
          if (res.result) {
      ...
      sendTestActivityReport: function sendTestActivityReport(email) {
        var self = this;
        this.ajax('wordfence_email_summary_email_address_debug', {
          email: email
        }, function (res) {
          if (res.result) {
      ...
      updateSwitch: function updateSwitch(elemID, configItem, cb) {
        var setting = jQuery('#' + elemID).is(':checked');
        this.updateConfig(configItem, jQuery('#' + elemID).is(':checked') ? 1 : 0, cb);
      },
      setupSwitches: function setupSwitches(elemID, configItem, cb) {
      ...
      updateConfig: function updateConfig(key, val, cb) {
        this.ajax('wordfence_updateConfig', {
          key: key,
          val: val
        }, function (ret) {
          if (cb) {
      ...
      updateIPPreview: function updateIPPreview(val, cb) {
      ...
      tourFinish: function tourFinish(page) {
        ...
        WFAD.ajax('wordfence_tourClosed', {
          page: page
        }, function (res) {});
      },
      tour: function tour(contentID, elemID, edge, align, previousCallback, nextCallback) {
        if (WFAD.currentPointer) {
          WFAD.currentPointer.pointer('destroy');
          WFAD.currentPointer = false;
        }
      ...
      switch2FAToNew: function switch2FAToNew() {},
      switch2FAToOld: function switch2FAToOld() {
        window.location.reload(true);
      },
      startActivityLogUpdates: function startActivityLogUpdates() {
        var self = this;
        setInterval(function () {
          self.updateActivityLog();
        }, parseInt(WordfenceAdminVars.actUpdateInterval));
      },
      updateActivityLog: function updateActivityLog() {
      ...
      doneUpdateActivityLog: function doneUpdateActivityLog(res) {
      ...
      updateSignaturesTimestamp: function updateSignaturesTimestamp(signatureUpdateTime) {
        ...
        if (sigTimestampEl.text() !== newText) {
          sigTimestampEl.text(newText).css({
            'opacity': 0
          }).animate({
            'opacity': 1
          }, 500);
      ...
      processActQueue: function processActQueue(currentScanID) {
      ...
      processActArray: function processActArray(arr) {
        for (var i = 0; i < arr.length; i++) {
          this.addActItem(arr[i]);
        }
      },
      addActItem: function addActItem(item) {
        if (!item) {
          return;
        }
      ...
      processSummaryLine: function processSummaryLine(item) {
      ...
      processActQueueItem: function processActQueueItem() {
        var item = this.activityQueue.shift();
      ...
      updateLiveTraffic: function updateLiveTraffic(forceUpdate) {
      ...
      handleLiveTrafficReturn: function handleLiveTrafficReturn(res) {
        this.liveTrafficUpdatePending = false;
        ...
        if (this.mode == 'liveTraffic') {
          ...
          if (_typeof(WFAD.wfLiveTraffic) !== undefined) {
      ...
      avatarLookup: function avatarLookup() {
        var ids = [];
      ...
      reverseLookupIPs: function reverseLookupIPs() {
        var self = this;
        var ips = [];
      ...
      killScan: function killScan(callback) {
      ...
      loadIssues: function loadIssues(callback, offset, limit) {
        if (this.mode != 'scan') {
          return;
        }
        ...
        var self = this;
        this.ajax('wordfence_loadIssues', {
          offset: offset,
          limit: limit
        }, function (res) {
      ...
      loadMoreIssues: function loadMoreIssues(callback, offset, limit, ignoredOffset, ignoredLimit) {
        offset = offset || 0;
        limit = limit || WordfenceAdminVars.scanIssuesPerPage;
      ...
      isIssueExpanded: function isIssueExpanded(issueID) {
      ...
      expandIssue: function expandIssue(issueID, makeVisible) {
        var key = 'wf-scan-issue-expanded-' + issueID;
      ...
      displayIssues: function displayIssues(res, callback) {
      ...
      appendIssues: function appendIssues(issues, callback) {
      ...
      appendIssue: function appendIssue(issueObject, container) {
      ...
      sortIssues: function sortIssues() {
      ...
      updateBulkButtons: function updateBulkButtons() {
      ...
      updateIssueCounts: function updateIssueCounts(issueCounts) {
      ...
      repositionSiteCleaningCallout: function repositionSiteCleaningCallout() {
      ...
      ajax: function ajax(action, data, cb, cbErr, noLoading) {
      ...
      colorbox: function colorbox(width, heading, body, settings) {
      ...
      colorboxModalHTML: function colorboxModalHTML(width, heading, body, settings) {
        if (typeof settings === 'undefined') {
          settings = {};
        }

        var prompt = $.tmpl(WordfenceAdminVars.modalHTMLTemplate, {
          title: heading,
          message: body
        });
      ...
      colorboxModal: function colorboxModal(width, heading, body, settings) {
        if (typeof settings === 'undefined') {
          settings = {};
        }

        var prompt = $.tmpl(WordfenceAdminVars.modalTemplate, {
          title: heading,
          message: body
        });
        var promptHTML = $("<div />").append(prompt).html();
      ...
      colorboxError: function colorboxError(errorMsg, isTokenError) {
        var callback = false;
      ...
      colorboxHTML: function colorboxHTML(width, html, settings) {
        if (typeof settings === 'undefined') {
          settings = {};
        }
      ...
      colorboxServiceQueue: function colorboxServiceQueue() {
      ...
      colorboxOpen: function colorboxOpen(width, html, settings) {
        var self = this;
      ...
      colorboxClose: function colorboxClose() {
        this.colorboxIsOpen = false;
        jQuery.wfcolorbox.close();
      },
      bulkOperationConfirmed: function bulkOperationConfirmed(op) {
        WFAD.colorboxClose();
      ...
      deleteFile: function deleteFile(issueID, force, callback) {
      ...
      promptToRepairFile: function promptToRepairFile(issueID, file) {
        if (window.localStorage) {
        ...
        WFAD.colorboxModalHTML(WFAD.isSmallScreen ? '300px' : '400px', __("Download Backup File"), __('Please make a backup of this file before proceeding. If you need to restore this backup file, you can copy it to the following path from your site\'s root:') + '<p class="wf-padding-add-top-medium"><code>' + file + '</code></p>' + '<a href="' + WFAD.makeDownloadFileLink(file) + '" onclick="jQuery(\'#wfRepairFileNextBtn\').prop(\'disabled\', false); return true;">' + __('Click here to download a backup copy of this file now') + '</a><p class="wf-flex-horizontal">' + '<input type="button" class="wf-btn wf-btn-primary" name="but1" id="wfRepairFileNextBtn" value="Repair File" disabled="disabled" onclick="WFAD.promptToRepairFileDone(' + parseInt(issueID, 10) + ', jQuery(\'#forceRepairFileCheckbox\').prop(\'checked\'));this.disabled=true;" />' + '<label class="wf-padding-add-left"><input type="checkbox" id="forceRepairFileCheckbox" onclick="jQuery(\'#wfRepairFileNextBtn\').prop(\'disabled\', !this.checked); return true;"> ' + __('Don\'t ask again') + '</label>' + '</p>' + '<div class="wordfenceHelpLink"><a href="' + WordfenceAdminVars.supportURLs['scan-result-repair-modified-files'] + '" target="_blank" rel="noopener noreferrer" class="wfhelp"></a><a href="' + WordfenceAdminVars.supportURLs['scan-result-repair-modified-files'] + '" target="_blank" rel="noopener noreferrer">' + __('Learn more about repairing modified files.') + ' (' + __('opens in new tab') + ')</a></div>');
      },
      promptToRepairFileDone: function promptToRepairFileDone(issueID, dontPromptAgain) {
        if (dontPromptAgain) {
          if (window.localStorage) {
      ...
      repairFile: function repairFile(issueID) {
        var self = this;
        self.colorboxClose();
      ...
      deleteDatabaseOption: function deleteDatabaseOption(issueID) {
        var self = this;
      ...
      doneDeleteDatabaseOption: function doneDeleteDatabaseOption(res) {
        var cb = false;
        var self = this;
      ...
      useRecommendedHowGetIPs: function useRecommendedHowGetIPs(issueID) {
        var self = this;
      ...
      fixFPD: function fixFPD(issueID) {
        var self = this;
      ...
      fixFPD_WriteHtAccess: function fixFPD_WriteHtAccess(issueID) {
        var self = this;
        self.colorboxClose();
      ...
      hideFile: function hideFile(issueID, callback) {
        WFAD.ajax('wordfence_checkHtaccess', {}, function (checkRes) {
          if (checkRes.ok) {
            WFAD.colorboxModalHTML(WFAD.isSmallScreen ? '300px' : '400px', __('.htaccess change'), __('We are about to change your <em>.htaccess</em> file. Please make a backup of this file before proceeding.') + '<br/>' + '<a id="dlButton" href="' + WordfenceAdminVars.ajaxURL + '?action=wordfence_downloadHtaccess&nonce=' + WFAD.nonce + '">' + __('Click here to download a backup copy of your .htaccess file now') + '</a>' + '<br /><br /><input type="button" class="wf-btn wf-btn-default" name="but1" id="wfFPDNextBut" value="' + __('Click to fix .htaccess') + '" disabled="disabled" />');
      ...
      restoreFile: function restoreFile(issueID, callback) {
      ...
      disableDirectoryListing: function disableDirectoryListing(issueID) {
        var self = this;
      ...
      confirmDisableDirectoryListing: function confirmDisableDirectoryListing(issueID) {
        var self = this;
        this.colorboxClose();
      ...
      deleteIssue: function deleteIssue(id) {
        var self = this;
        this.ajax('wordfence_deleteIssue', {
          id: id
        }, function (res) {
          self.loadIssues();
        });
      },
      updateIssueStatus: function updateIssueStatus(id, st, callback) {
        this.ajax('wordfence_updateIssueStatus', {
          id: id,
          'status': st
        }, function (res) {
          typeof callback === 'function' && callback(res);
      ...
      noQuotes: function noQuotes(str) {
        return str.replace(/"/g, '&#34;').replace(/\'/g, '&#145;');
      },
      commify: function commify(num) {
        return ("" + num).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
      },
      switchToLiveTab: function switchToLiveTab(elem) {
      ...
      completeLiveTabSwitch: function completeLiveTabSwitch() {
      ...
      emailActivityLog: function emailActivityLog() {
      ...
      completeEmailActivityLog: function completeEmailActivityLog() {
        WFAD.colorboxClose();
        ...
        var self = this;
        this.ajax('wordfence_sendActivityLog', {
          email: jQuery('#wfALogRecip').val()
        }, function (res) {
          if (res.ok) {
      ...
      reloadActivityData: function reloadActivityData() {
        jQuery('#wfActivity').html('<div class="wfLoadingWhite32"></div>'); //&nbsp;<br />&nbsp;<br />&nbsp;<br />&nbsp;<br />&nbsp;<br />&nbsp;<br />&nbsp;<br />&nbsp;<br />&nbsp;<br />&nbsp;<br />

        this.completeLiveTabSwitch();
      },
      switchToSummaryTab: function switchToSummaryTab(elem) {
        jQuery('.wfTab1').removeClass('selected');
        jQuery(elem).addClass('selected');
        jQuery('.wfDataPanel').hide();
        jQuery('#wfSummaryTables').fadeIn();
      },
      switchIssuesTab: function switchIssuesTab(elem, type) {
      ...
      switchTab: function switchTab(tabElement, tabClass, contentClass, selectedContentID, callback) {
      ...
      reloadLiveTraffic: function reloadLiveTraffic() {
      ...
      makeIPTrafLink: function makeIPTrafLink(IP) {
        return WordfenceAdminVars.siteBaseURL + '?_wfsf=IPTraf&nonce=' + this.nonce + '&IP=' + encodeURIComponent(IP);
      },
      makeDiffLink: function makeDiffLink(dat) {
        return WordfenceAdminVars.siteBaseURL + '?_wfsf=diff&nonce=' + this.nonce + '&file=' + encodeURIComponent(this.es(dat['file'])) + '&cType=' + encodeURIComponent(this.es(dat['cType'])) + '&cKey=' + encodeURIComponent(this.es(dat['cKey'])) + '&cName=' + encodeURIComponent(this.es(dat['cName'])) + '&cVersion=' + encodeURIComponent(this.es(dat['cVersion']));
      },
      makeViewFileLink: function makeViewFileLink(file) {
        return WordfenceAdminVars.siteBaseURL + '?_wfsf=view&nonce=' + this.nonce + '&file=' + encodeURIComponent(file);
      },
      makeDownloadFileLink: function makeDownloadFileLink(file) {
        return WordfenceAdminVars.siteBaseURL + '?_wfsf=download&nonce=' + this.nonce + '&file=' + encodeURIComponent(file);
      },
      makeViewOptionLink: function makeViewOptionLink(option, siteID) {
      ...
      parseHashes: function parseHashes() {
      ...
      makeTimeAgo: function makeTimeAgo(t) {
      ...
      pluralize: function pluralize(m1, t1, m2, t2) {
        if (m1 != 1) {
          t1 = t1 + 's';
        }
      ...
      isValidIP: function isValidIP(ip) {
        ...
        if (ip.match(/^(?:\d{1,3}(?:\.|$)){4}/)) {
        ...
        } else if (ip.match(/^((?:[\da-f]{1,4}(?::|)){0,8})(::)?((?:[\da-f]{1,4}(?::|)){0,8})$/i)) {
          //IPv6
          if (ip == '::') {
            return true;
          }
          ...
          if (doubleColonPos > -1) {
            ...
            if (expansionLength == 0) {
              //Double-colon in a full IPv6 address
              return false;
            }
      ...
      parseIPRange: function parseIPRange(range) {
        if (!range) {
          return false;
        }
        ...
        if (range && /^[^\-]+\-[^\-]+$/.test(range)) {
          ...
          for (var i = 0; i < binStart.length; i++) {
            ...
            if (i < 11 && n1 - n0 > 0) {
              //Based on Number.MAX_SAFE_INTEGER, which equals 2 ^ 53 - 1. Any of the first 9 bytes and part of the 10th that add to the range will put us over that
              countOverflow = true;
              break;
      ...
      calcRangeTotal: function calcRangeTotal() {
      ...
      completeWhois: function completeWhois(res, ret) {
        ret = ret === undefined ? false : !!ret;
        var self = this;
        var rawhtml = "";
        var ipRangeTmpl = jQuery("<div><div class='wf-flex-row'>" + "<a class=\"wf-btn wf-btn-default wf-flex-row-0\" href=\"${adminUrl}\">" + __('Block This Network') + "</a>" + "<span class='wf-flex-row-1 wf-padding-add-left'>{{html totalStr}}{{if totalStr.indexOf(ipRange) == -1}} (${ipRange}){{/if}}" + '{{if (totalIPs)}}<br>[' + __('${totalIPs} addresses in this network') + ']{{/if}}' + "</span></div></div>");

        if (res.ok && res.result && res.result.rawdata && res.result.rawdata.length > 0) {
          for (var i = 0; i < res.result.rawdata.length; i++) {
            var wfm21 = function wfm21(str, startStr, ipRange, offset, totalStr) {
            ...
            var buildRangeLink2 = function buildRangeLink2(str, startStr, octet1, octet2, octet3, octet4, cidrRange, offset, totalStr) {
      ...
      blockIPUARange: function blockIPUARange(ipRange, hostname, uaRange, referer, reason) {
        if (!/\w+/.test(reason)) {
      ...
      blockIP: function blockIP(IP, reason, callback) {
        var self = this;
      ...
      unlockOutIP: function unlockOutIP(IP) {
        var self = this;
      ...
      unblockIP: function unblockIP(IP, callback) {
        var self = this;
      ...
      unblockNetwork: function unblockNetwork(id) {
        var self = this;
      ...
      unblockIPTwo: function unblockIPTwo(IP) {
        var self = this;
        this.ajax('wordfence_unblockIP', {
          IP: IP
        }, function (res) {
      ...
      permBlockIP: function permBlockIP(IP) {
        var self = this;
      ...
      makeElemID: function makeElemID() {
        return 'wfElemGen' + this.elementGeneratorIter++;
      },
      pulse: function pulse(sel) {
      ...
      twoFacStatus: function twoFacStatus(msg) {
        this.colorboxModal('300px', __('Two Factor Status'), msg);
      },
      addTwoFactor: function addTwoFactor(username, phone, mode) {
        var self = this;
        this.ajax('wordfence_addTwoFactor', {
          ...
          if (res.ok) {
              ...
              self.colorboxModalHTML(self.isSmallScreen ? '300px' : '440px', __("Authentication Code"), message, {
                onComplete: function onComplete() {
                  ...
                  jQuery('#wfTwoFactorDownload').on('click', function (e) {
                    ...
                    }), self.htmlEscape(res.homeurl) + "_" + self.htmlEscape(res.username) + "_recoverycodes.txt");
                  });
                }
              });
            } else {
            ...
            var updatedTwoFac = jQuery('#wfTwoFacUserTmpl').tmpl({
              users: [res]
            });
      ...
      twoFacActivate: function twoFacActivate(userID, code) {
        var self = this;
      ...
      delTwoFac: function delTwoFac(userID) {
          ...
          if (res.ok) {
            jQuery('#twoFactorUser-' + res.userID).fadeOut(function () {
              ...
              if (jQuery('#wfTwoFacUsers > table > tbody:last-child').children().length == 0) {
                jQuery('#wfTwoFacUsers').html(jQuery('#wfTwoFacUserTmpl').tmpl({
                  users: []
                }));
              }
            });
      ...
      loadTwoFactor: function loadTwoFactor() {
      ...
      inet_aton: function inet_aton(dot) {
        var d = dot.split('.');
        return ((+d[0] * 256 + +d[1]) * 256 + +d[2]) * 256 + +d[3];
      },
      inet_ntoa: function inet_ntoa(num) {
        var d = num % 256;
      ...
      deleteAdminUser: function deleteAdminUser(issueID) {
        var self = this;
      ...
      revokeAdminUser: function revokeAdminUser(issueID) {
        var self = this;
      ...
      permanentlyBlockAllIPs: function permanentlyBlockAllIPs(type) {
        var self = this;
      ...
      showTimestamp: function showTimestamp(timestamp, serverTime, format) {
      ...
      wafWhitelistedBulkChangeEnabled: function wafWhitelistedBulkChangeEnabled(enabled) {
      ...
      wafWhitelistedChangeEnabled: function wafWhitelistedChangeEnabled(key, enabled) {
      ...
      wafWhitelistedBulkDelete: function wafWhitelistedBulkDelete() {
        $('.wf-whitelist-table-bulk-checkbox.wf-option-checkbox.wf-checked').each(function () {
          $(this).closest('tr').find('.wf-whitelist-item-enabled.wf-option-checkbox').each(function () {
            var tr = $(this).closest('tr');
      ...
      wafWhitelistedDelete: function wafWhitelistedDelete(key) {
        $('#waf-whitelisted-urls-wrapper .whitelist-table > tbody > tr[data-key="' + key + '"]').each(function () {
          var adding = !!$(this).data('adding');
      ...
      wafConfigPageRender: function wafConfigPageRender() {
      ...
      renderWAFRulesLastUpdated: function renderWAFRulesLastUpdated(date) {
        var dateString = date.toString();
        ...
        $('#waf-rules-last-updated').text(sprintf(__('Last Updated: %s'), dateString)).css({
          'opacity': 0
        }).animate({
          'opacity': 1
        }, 500);
      },
      renderWAFRulesNextUpdate: function renderWAFRulesNextUpdate(date) {
        var dateString = date.toString();
        ...
        $('#waf-rules-next-update').text(sprintf(__('Next Update Check: %s'), dateString)).css({
          'opacity': 0
        }).animate({
          'opacity': 1
        }, 500);
      },
      wafUpdateRules: function wafUpdateRules(onSuccess) {
        var self = this;
      ...
      confirmWAFConfigureAutoPrepend: function confirmWAFConfigureAutoPrepend() {
        var self = this;
      ...
      updatePendingChanges: function updatePendingChanges() {
      ...
      _unsavedOptionsHandler: function _unsavedOptionsHandler(e) {
      ...
      setOption: function setOption(key, value, successCallback, failureCallback) {
        var changes = {};
        changes[key] = value;
      ...
      saveOptions: function saveOptions(successCallback, failureCallback) {
        if (!Object.keys(WFAD.pendingChanges).length) {
          return;
        }
      ...
      enableAllOptionsPage: function enableAllOptionsPage() {
  ...
  $(function () {
      ...
      WFAD.colorboxHTML(WFAD.isSmallScreen ? '300px' : '400px', promptHTML, {
        ...
        onComplete: function onComplete() {
          ...
          $('#wf-restore-defaults-prompt-confirm').on('click', function (e) {
            e.preventDefault();
            e.stopPropagation();
            WFAD.ajax('wordfence_restoreDefaults', {
              section: restoreDefaultsSection
            }, function (res) {
              if (res.success) {
                window.location.reload(true);
              } else {
    ...
    $('#wf-cancel-changes').on('click', function (e) {
      ...
      e.stopPropagation();
      WFAD.pendingChanges = {};
      WFAD.updatePendingChanges(); //On/Off options
          ...
          circle.tooltip({
            ...
            position: {
              ...
              using: function using(obj, info) {
                var el = $(this);
                el.removeClass('wf-tooltip-vertical-top wf-tooltip-vertical-bottom ' + 'wf-tooltip-horizontal-left wf-tooltip-horizontal-right').addClass('wf-tooltip-vertical-' + info.vertical).addClass('wf-tooltip-horizontal-' + info.horizontal);
                $(this).css({
                  left: obj.left + 'px',
                  top: obj.top + 'px'
                });
...
jQuery.fn.wfCircularProgress = function (options) {
  ...
  jQuery(this).each(function () {
    ...
    if (_typeof(creationOptions) !== 'object') {
      creationOptions = {};
    }
...
})(jQuery, document, window); //wfMobileMenu
...
(function ($, document, window) {
  ...
  var publicMethod = $.fn['wfMobileMenu'] = $['wfMobileMenu'] = function (options) {
    ...
    for (var i = 0; i < opts.menuItems.length; i++) {
      ...
      button.find('a').text(opts.menuItems[i].title).css('width', opts.width).on('click', null, {
        action: opts.menuItems[i].action
      }, function (e) {
        e.preventDefault();
        e.stopPropagation();
...
!function (t) {
  ...
  var n = t.BlobBuilder || t.WebKitBlobBuilder || t.MozBlobBuilder || function (t) {
    ...
    return u.createObjectURL || (R = t.URL = function (t) {
      ...
      if (g && (t instanceof b || t instanceof g)) {
        for (var i = "", a = new g(t), l = 0, s = a.length; s > l; l++) {
  ...
  t.Blob = function (t, e) {
    var o = e ? e.type || "" : "",
        i = new n();
    if (t) for (var a = 0, r = t.length; r > a; a++) {
      Uint8Array && t[a] instanceof Uint8Array ? i.append(t[a].buffer) : i.append(t[a]);
    }
    var c = i.getBlob(o);
