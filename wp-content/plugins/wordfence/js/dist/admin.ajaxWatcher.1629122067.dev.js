...
!function (t, e, i) {
  ...
  function p() {
    x || (V = !1, E = t(i), x = n(st).attr({
      ...
      role: "dialog",
      tabindex: "-1"
    }).hide(), v = n(st, "Overlay").hide(), L = t([n(st, "LoadingOverlay")[0], n(st, "LoadingGraphic")[0]]), y = n(st, "Wrapper"), b = n(st, "Content").append(F = n(st, "Title"), R = n(st, "Current"), K = t('<button type="button"/>').attr({
      id: Z + "Previous"
    }), B = t('<button type="button"/>').attr({
      id: Z + "Next"
    }), S = t('<button type="button"/>').attr({
      id: Z + "Slideshow"
    }), L), P = t('<button type="button"/>').attr({
      id: Z + "Close"
      ...
      "float": "left"
    }), M = n(st, !1, "position:absolute; width:9999px; visibility:hidden; display:none; max-width:none;"), O = B.add(K).add(R).add(S)), e.body && !x.parent().length && t(e.body).append(v, x.append(y, M));
  }
    ...
    window['wordfenceAJAXWatcher'] = {
      ...
      init: function init() {
        $(document).ajaxError(function (event, jqXHR, ajaxSettings, thrownError) {
          ...
          if (jqXHR.responseJSON) {//Do nothing
          } else {
            ...
            if (nonceMatches && nonceMatches[1] == WFAJAXWatcherVars.nonce) {
              ...
              if (!(typeof formAction === "string")) {
                //Only progress if it's our plugin doing the blocking
                return;
              }
