...
(function ($) {
  window['WFLS'] = {
    ...
    init: function init() {
      ...
      }).triggerHandler('change'); //Text area option
      ...
      $('#wfls-cancel-changes').on('click', function (e) {
        e.preventDefault();
        e.stopPropagation(); //On/Off options
    ...
    setOptions: function setOptions(options, successCallback, failureCallback) {
      if (!Object.keys(options).length) {
        return;
      }

      this.ajax('wordfence_ls_save_options', {
    ...
    updateIPPreview: function updateIPPreview(value, successCallback) {
     ...
     */
    panel: function panel(width, heading, body, settings) {
     ...
     * @param object settings
     */
    panelModalHTML: function panelModalHTML(width, heading, body, settings) {
      if (typeof settings === 'undefined') {
        settings = {};
      }

      var prompt = $.tmpl(WFLSVars.modalHTMLTemplate, {
        title: heading,
        message: body
      });
     ...
     * @param object settings
     */
    panelModal: function panelModal(width, heading, body, settings) {
      if (typeof settings === 'undefined') {
        settings = {};
      }

      var prompt = $.tmpl(WFLSVars.modalTemplate, {
        title: heading,
        message: body
      });
      ...
      if (typeof settings.additional_buttons !== 'undefined') {
        ...
        for (index in settings.additional_buttons) {
          ...
          var button = $('<button>').text(buttonSettings.label).addClass('wfls-btn wfls-btn-default wfls-btn-callout-subtle wfls-additional-button').attr('id', buttonSettings.id);
     ...
     */
    panelError: function panelError(errorMsg, isTokenError) {
      var callback = false;
      ...
      var prompt = $.tmpl(WFLSVars.tokenInvalidTemplate, {
     ...
     * @param object settings
     */
    panelHTML: function panelHTML(width, html, settings) {
      if (typeof settings === 'undefined') {
        settings = {};
      }
     ...
     */
    _panelServiceQueue: function _panelServiceQueue() {
      if (WFLS.panelIsOpen) {
        return;
      }
     ...
     * @param object settings
     */
    _panelOpen: function _panelOpen(width, html, settings) {
