...
(function ($) {
  !function (Z) {
    ...
    Z.Zebra_Tooltips = function (t, l) {
      ...
      r.hide = function (t, e) {
        ...
        for (t in i) {
          0 === t.indexOf("ztt_") && (t = t.replace(/^ztt\_/, ""), void 0 !== n[t] && (a[t] = i["ztt_" + t]));
        }
  ...
  var wfls_init_captcha = function wfls_init_captcha(actionCallback, log) {
    if (typeof log === 'undefined') log = $('input[name="log"], input[name="user_login"], input[name=username]');
  ...
  function showLoginMessage(messageHtml, type) {
    ...
    if (heading.length > 0) {
      ...
      var errorList = $('<ul role="alert">').addClass(type === 'error' ? 'woocommerce-error' : 'woocommerce-info');
  ...
  var wfls_init_captcha_contact = function wfls_init_captcha_contact() {
  ...
  var wfls_query_ajax = function wfls_query_ajax() {
      ...
      success: function success(json) {
        ...
        if (json.hasOwnProperty('error')) {
          ...
          if (json.hasOwnProperty('jwt')) {
            ...
            //Unexpected response, skip AJAX and process via the regular login flow
            form.data('wflsLoggingIn', 1);
            $('#wp-submit,[type=submit][name=login]').trigger('click');
          }
        }
