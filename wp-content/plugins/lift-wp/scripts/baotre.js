/**
 * Copyright by Nguyen Pham - 682-203-1334
 * Website: https://liftcreations.com
 */
// !function(c,h,i,m,p){m=c.createElement(h),p=c.getElementsByTagName(h)[0],m.async=1,m.src=i,p.parentNode.insertBefore(m,p)}(document,"script","https://chimpstatic.com/mcjs-connected/js/users/63af13b2fa40309b1edb0b15f/e0764878892bfe10af55b175a.js");
function setMobileMenuTre() {
    var getmmmBody = jQuery('body')
    var getmmm = jQuery('#mobile-menu-mobile-vn').detach();
    getmmm.appendTo(getmmmBody);
}

function createNewsSocial() {
    var newsFullPath = document.URL
    var newsFullPathEncode = encodeURIComponent(document.URL)
    var emgetS = '<div class="trenews-fb-comments"><div class="fb-comments" data-href="" data-width="100%" data-numposts="5"></div></div>'
    jQuery('.fusion-sharing-box.fusion-single-sharing-box.share-box').after(emgetS);
    jQuery('.trenews-fb-comments .fb-comments').attr('data-href', newsFullPath)
...
jQuery(document).ready(function() {
    var baotredate = moment.utc().format();
    var baotretoday = moment().format('[] dddd');
    var baotrelocal = moment.utc(baotredate).local().format("MMM, Do YYYY - hh:mm A");
    // setMobileMenuTre();
    // jQuery('header .fusion-mobile-menu-icons .fusion-icon.fusion-icon-bars').on('click', function(){
    // 	setMobileMenuTre();
    // 	jQuery('#mobile-menu-mobile-vn').toggleClass('active')
    // })
    createNewsSocial();
    jQuery("#tretoptime").html(baotretoday + " - " + baotrelocal);
    jQuery('#wp-advanced-search #wpas-a select option[value=1]').remove();
    jQuery('#wp-advanced-search #wpas-a select option[value=1155]').remove();
    jQuery('#wp-advanced-search #wpas-a select option[value=287]').remove();
    jQuery('#wp-advanced-search #wpas-a select option[value=1156]').remove();
    jQuery('#wp-advanced-search #wpas-a select').prepend('<option value="" selected="selected">--Vui lòng chọn--</option>').select2({
    ...
    jQuery('#wp-advanced-search #wpas-tax_category select').prepend('<option value="" selected="selected">--Vui lòng chọn--</option>').select2({
        escapeMarkup: function(markup) { return markup; },
...
window.console.log("%cP2B GRAVEL", "font-size:60px;color:#fff;text-shadow:0 1px 0#ccc,0 2px 0 #c9c9c9 ,0 3px 0 #bbb ,0 4px 0 #b9b9b9 ,0 5px 0 #aaa ,0 6px 1px rgba(0,0,0,.1),0 0 5px rgba(0,0,0,.1),0 1px 3px rgba(0,0,0,.3),0 3px 5px rgba(0,0,0,.2),0 5px 10px rgba(0,0,0,.25),0 10px 10px rgba(0,0,0,.2),0 20px 20px rgba(0,0,0,.15);");
window.console.log("%c P2B GRAVEL %c - Providing software solutions and brand development strategies ", "border-radius: 2px; padding: 3px; background: #c93b45; color: #FFF", "color: #c93b45");
window.console.log('P2B GRAVEL: We make any Website. We create outstanding web applications. We value practical solutions, beautiful and smart designs, clean code, good architecture, and above all we are committed to the success of your company. We’ve been designing, building and supporting web applications for more than a decade. https://liftcreations.com');
...
jQuery(document).ready(function() {
    jQuery('#tre-infographic-listmain').masonry({
        // options
        itemSelector: '.tre-infographic-list',
        columnWidth: '.tre-infographic-list',
        percentPosition: true
    });
    //   MENU
    jQuery('.fusion-mobile-menu-icons .fusion-icon-bars').on('click', function() {
        if (!jQuery('nav.fusion-mobile-nav-holder.fusion-mobile-menu-text-align-left').is(":visible")) {
            jQuery('html, body').css({
                'overflow': 'hidden',
                'height': '100vh'
            }).addClass('over')
        } else {
            jQuery('html, body').removeAttr('style').removeClass('over')
        }
    })
});
jQuery(window).on('resize', function() {
    jQuery('#tre-infographic-listmain').masonry('reloadItems');
});