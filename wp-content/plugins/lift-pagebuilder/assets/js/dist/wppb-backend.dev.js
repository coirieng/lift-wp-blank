...
jQuery(document).ready(function ($) {
  ...
  jQuery('.edit-with-wppb-builder').click(function (e) {
    ...
    $.post(wppb_admin_ajax.ajax_url, {
      action: 'wppb_switch_editor',
      post_id: post_id,
  ...
  $(document).on('click', '#wppb_clear_cache_btn', function (e) {
    ...
    jQuery.ajax({
      ...
      success: function success(response) {
        $that.closest('label').find('.response-text').html('<span style="color: #228b22;">LIFT WP Page Builder cache has been cleared.</span>');
      },
  ...
  var wppbGutenbergCompatibility = {
    init: function init() {
      var self = this;
      setTimeout(function () {
        self.addWPPBBtn();
    ...
    addWPPBBtn: function addWPPBBtn() {
      var isBtnExists = this.toolbarSelector.find('#wppb-edit-with-btn-in-gutenberg-toolbar').length;
      var btnHtmlWrap = $('#wppb-edit-with-btn-in-gutenberg-toolbar');

      if (!isBtnExists && btnHtmlWrap.length) {
        $('.edit-post-header__toolbar').append(btnHtmlWrap.html());
      }
    },
    addSwitchTemplate: function addSwitchTemplate() {
      var hasSwitchTemplate = $('#lift-pagebuilder-to-gutenberg-switch-mode').length;

      if (hasSwitchTemplate) {
        $('.editor-block-list__layout').after($('#lift-pagebuilder-to-gutenberg-switch-mode').html());
      }
    }
  };
