...
jQuery(document).ready(function ($) {
  ...
  $(document).on('rendered_addon', function (e, addon) {
    var iframe = window.frames['wppb-builder-view'].window.document;
  ...
  $(document).on('click', '.wppb-accordion-title', function () {
    var $items = $(this).closest('.wppb-accordion-items').find('>div');
    var $handlers = $items.find('.wppb-accordion-title');
    var $panels = $items.find('.wppb-panel-collapse');
  ...
  $(document).on('rendered_addon', function (e, addon) {
    if (typeof addon.type !== 'undefined' && (addon.type === 'addon' || addon.type === 'inner_addon') && (addon.name === 'wppb_testimonial_carousel' || addon.name === 'wppb_person_carousel')) {
      ...
      if (addon.name == 'wppb_person_carousel') {
        Container = $(iframe).find('.wppb-person-content-carousel');
      }

      if (addon.name == 'wppb_testimonial_carousel') {
        Container = $(iframe).find('.wppb-testimonial-content-carousel');
      }
  ...
  var testimonial = $('.wppb-testimonial-content-carousel');
  var person = $('.wppb-person-content-carousel');
      ...
      fire_live_change: function fire_live_change() {
        $(document).on('widget-added', function () {
          $('.wppb-form-widget').on('change', function () {
            var wppb_form_widget = $('.wppb-form-widget');
            var widgetFormInput = wppb_form_widget.closest('form').serializeArray();

            if (wppb_form_widget.length) {
              var widgetFormData = {};
              var id_base = wppb_form_widget.find('[name="id_base"]').val();
              var widget_input_base_name = jQuery('[name="widget_input_base_name"]').val();
              var wppb_widget_id = wppb_form_widget.find('[name="widget-id"]').val();
