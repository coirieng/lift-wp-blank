...
if ( ! class_exists('WPPB_Frontend')){
	...
	class WPPB_Frontend{
			...
			if ( ! wppb_helper()->is_load_editor_iframe()) {
				...
				if ( $post_id ) {
					...
					if ( $css_save_as === 'filesystem' ) {
						...
						$css_path       = $upload_css_dir . "lift-pagebuilder/wppb-page-css-{$post_id}.css";
						if ( file_exists( $css_path ) ) {
							$css_dir_url = trailingslashit( $upload_dir['baseurl'] );
							$css_url     = $css_dir_url . "lift-pagebuilder/wppb-page-css-{$post_id}.css";
		 ...
		 * @return array
		 *
		 * Add Post Class in single page for LIFT WP Page Builder
		 */
		 ...
		 * @return array
		 *
		 * Add Body Class in single page for LIFT WP Page Builder
		 */
			...
			if (is_singular()){
				...
				if ( in_array($post->post_type, $wppb_supports_post_types)) {
					...
					$admin_bar->add_menu(
						array(
							'id'    => 'edit-with-wppb',
							'title' => __( 'Edit with LIFT WP Page Builder', 'lift-pagebuilder' ),
							'href'  => $link,
							'meta'  => array(
								'title' => __( 'Edit with LIFT WP Page Builder', 'lift-pagebuilder' ),
							),
