...
if ( ! class_exists('WPPB_Ajax')){
	...
	class WPPB_Ajax{
		...
		public function __construct() {
			...
			//Get Template from LIFT WP Page Builder Server
		...
		public function  wppb_page_save(){
			...
			$success_array = array( 'msg' => __('LIFT WP Page Builder content has been updated.', 'lift-pagebuilder') );
			$error_array = array();

			$filename = "wppb-page-css-{$page_id}.css";

			$upload_dir = wp_upload_dir();
			$dir = trailingslashit($upload_dir['basedir']) . 'lift-pagebuilder/';
			...
			if ( ! $wp_filesystem->put_contents( $dir . $filename, $wppb_page_css ) ) {
				$error_array['msg_error'] = __('CSS can not be saved due to permission!!!', 'lift-pagebuilder');
			}
		...
		/**
		 * @since 1.0.0
		 * Delete css file generated by LIFT WP Page Builder
		 */
		...
		public function wppb_clear_cache(){
			...
			$upload_dir     = wp_upload_dir();
			$dirPattern     = trailingslashit($upload_dir['basedir']).'lift-pagebuilder/cache/';
