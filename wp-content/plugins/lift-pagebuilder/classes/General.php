	...
	class WPPB_General{
		...
		public function posts_table_row_actions( $actions, $post ) {
			...
			if ( in_array( $post->post_type, $wppb_supports_post_types ) ) {
				...
				$actions['wppb_editor'] = '<a href="'.$editor_url.'">'.__('Edit with LIFT WP Page Builder', 'lift-pagebuilder' ).'</a>';
			}
			return $actions;
		}
				...
				if ( $screen->base === 'edit'){
					...
					if ( in_array($post->post_type, $wppb_supports_post_types)) {
						...
						if ( $is_wppb_editor_page ) {
							$post_states[] = ' LIFT WP Page Builder ';
						}
					}
				}
			}
			...
			<?php if( $editor == 'wppb_builder_activated' ) { ?>
				<div class="wppb-edit-button-wrap">
					<div class="wppb-back-editor wppb-class-editor-show">
						<a href="#" class="wppb-backend-btn wppb-info-btn use-default-editor"><?php _e('Use Default Editor','lift-pagebuilder' );?></a>
					</div><!--/.wppb-back-editor-->
					<div class="wppb-editor-warper">
						<a href="<?php echo wppb_helper()->get_editor_url($post->ID); ?>" class="wppb-backend-btn wppb-primary-btn edit-with-wppb-builder"><?php _e('Edit With LIFT WP Page Builder','lift-pagebuilder');?></a>
					</div>
		...
		public function wppb_add_admin_menu(){
            $is_pro_activated = is_plugin_active('lift-pagebuilder-pro/lift-pagebuilder-pro.php');
            if ( ! $is_pro_activated ){
	            // add_submenu_page( 'lift-wppb-settings', __( 'Go Premium', 'lift-pagebuilder' ), __( '<span class="dashicons dashicons-awards wppb-go-premium-star"></span> Go Premium', 'lift-pagebuilder' ), 'manage_options', 'wppb-go-premium', array($this, 'wppb_go_premium_page') );
            }
		}
