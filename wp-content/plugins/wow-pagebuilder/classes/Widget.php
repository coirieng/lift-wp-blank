	...
	class WOW_Widget {
		...
		*/
		public function get_id($settings  = array()){
            ...
            }elseif( ! empty($_POST['widget']['settings']['wow_widget_id']) ) {
			    $widget_id = sanitize_text_field( $_POST['widget']['settings']['wow_widget_id'] );
		    }elseif( ! empty($settings['wow_widget_id'])){
			    $widget_id = $settings['wow_widget_id'];
            }else{
        ...
        private function get_widget_id_base($settings = array()){
            ...
            }elseif( ! empty($_POST['widget']['settings']['wow_widget_id_base'] )){
                $widget_id_base = sanitize_text_field($_POST['widget']['settings']['wow_widget_id_base']);
            }
            if (is_null($widget_id_base)){
            	if ( ! empty($settings['wow_widget_id_base'])){
		            $widget_id_base = $settings['wow_widget_id_base'];
	            }
            }
		...
		*/
		public function get_form($settings = array()) {
			...
			echo '<div class="widget-inner widget-inside media-widget-control">';
				echo '<div class="form wp-core-ui wow-form-widget">';
