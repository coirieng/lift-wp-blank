...
!function ($) {
  ...
  $.fn.vcRoundChart = function () {
    ...
    return this.each(function () {
      ...
      for ($this.data("chart") && ($this.data("chart").destroy(), $this.removeData("animated")), data = $this.data("vcValues"), ctx.canvas.width = $this.width(), ctx.canvas.height = $this.width(), i = data.length - 1; 0 <= i; i--) {
        for (j = color_keys.length - 1; 0 <= j; j--) {
          "object" == _typeof(data[i][color_keys[j]]) && 2 === data[i][color_keys[j]].length && ((gradient = ctx.createLinearGradient(0, 0, 0, ctx.canvas.height)).addColorStop(0, data[i][color_keys[j]][0]), gradient.addColorStop(1, data[i][color_keys[j]][1]), data[i][color_keys[j]] = gradient);
        }
      }
    ...
    $(selector = void 0 !== model_id ? '[data-model-id="' + model_id + '"] ' + selector : selector).vcRoundChart();
