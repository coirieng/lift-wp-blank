  ...
  $.flexslider = function (el, options) {
    ...
    $.data(el, "flexslider", slider), methods = {
      ...
      controlNav: {
        ...
        setupPaging: function setupPaging() {
          var item,
              ...
              j = 1;
          if (slider.controlNavScaffold = $('<ol class="' + namespace + "control-nav " + namespace + type + '"></ol>'), 1 < slider.pagingCount) for (var captn, i = 0; i < slider.pagingCount; i++) {
      ...
      touch: function touch() {
        ...
        msGesture ? (el.style.msTouchAction = "none", el._gesture = new MSGesture(), (el._gesture.target = el).addEventListener("MSPointerDown", function (e) {
        ...
        }, !1)) : (onTouchStart = function onTouchStart(e) {
          slider.animating ? e.preventDefault() : !window.navigator.msPointerEnabled && 1 !== e.touches.length || (slider.pause(), cwidth = vertical ? slider.h : slider.w, startT = Number(new Date()), localX = e.touches[0].pageX, localY = e.touches[0].pageY, offset = carousel && reverse && slider.animatingTo === slider.last ? 0 : carousel && reverse ? slider.limit - (slider.itemW + slider.vars.itemMargin) * slider.move * slider.animatingTo : carousel && slider.currentSlide === slider.last ? slider.limit : carousel ? (slider.itemW + slider.vars.itemMargin) * slider.move * slider.currentSlide : reverse ? (slider.last - slider.currentSlide + slider.cloneOffset) * cwidth : (slider.currentSlide + slider.cloneOffset) * cwidth, startX = vertical ? localY : localX, startY = vertical ? localX : localY, el.addEventListener("touchmove", onTouchMove, !1), el.addEventListener("touchend", _onTouchEnd, !1));
          ...
          var updateDx, target;
          el.removeEventListener("touchmove", onTouchMove, !1), slider.animatingTo !== slider.currentSlide || scrolling || null === dx || (target = 0 < (updateDx = reverse ? -dx : dx) ? slider.getTarget("next") : slider.getTarget("prev"), slider.canAdvance(target) && (Number(new Date()) - startT < 550 && 50 < Math.abs(updateDx) || Math.abs(updateDx) > cwidth / 2) ? slider.flexAnimate(target, slider.vars.pauseOnAction) : fade || slider.flexAnimate(slider.currentSlide, slider.vars.pauseOnAction, !0)), el.removeEventListener("touchend", _onTouchEnd, !1), offset = dx = startY = startX = null;
      ...
      var posCheck,
          target = (posCheck = pos || (slider.itemW + slider.vars.itemMargin) * slider.move * slider.animatingTo, function () {
    ...
    if ("object" == _typeof(options = void 0 === options ? {} : options)) return this.each(function () {
      var $this = $(this),
