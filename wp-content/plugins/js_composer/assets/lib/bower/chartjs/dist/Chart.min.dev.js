...
!function () {
  ...
  function Chart(context) {
    function computeDimension(element, dimension) {
      return element["offset" + dimension] || document.defaultView.getComputedStyle(element).getPropertyValue(dimension);
    }

    return this.canvas = context.canvas, this.ctx = context, this.width = computeDimension(context.canvas, "Width") || context.canvas.width, this.height = computeDimension(context.canvas, "Height") || context.canvas.height, this.aspectRatio = this.width / this.height, helpers.retinaScale(this), this;
  }
  ...
  var id,
    ...
    if (loopable) if (loopable.length === +loopable.length) for (var i = 0; i < loopable.length; i++) {
    ...
    for (var i = (startIndex = startIndex || -1) + 1; i < arrayToSearch.length; i++) {
      var currentItem = arrayToSearch[i];
      if (filterCallback(currentItem)) return currentItem;
    }
  }, helpers.findPreviousWhere = function (arrayToSearch, filterCallback, startIndex) {
    for (var i = (startIndex = startIndex || arrayToSearch.length) - 1; 0 <= i; i--) {
      var currentItem = arrayToSearch[i];
      if (filterCallback(currentItem)) return currentItem;
    }
    ...
    return degrees * (Math.PI / 180);
      ...
      aliasPixel = (helpers.getAngleFromPoint = function (angle, radialDistanceFromCenter) {
    ...
    var d01 = Math.sqrt(Math.pow(MiddlePoint.x - FirstPoint.x, 2) + Math.pow(MiddlePoint.y - FirstPoint.y, 2)),
        fb = Math.sqrt(Math.pow(AfterPoint.x - MiddlePoint.x, 2) + Math.pow(AfterPoint.y - MiddlePoint.y, 2)),
        fa = t * d01 / (d01 + fb),
        fb = t * fb / (d01 + fb);
    return {
      inner: {
    ...
    return Math.floor(Math.log(val) / Math.LN10);
      ...
      template = (helpers.calculateScaleRange = function (valueRange, graphMin, graphMax, startFromZero, integersOnly) {
    ...
    var fn,
        data,
        cache = {};
    return data = valuesObject, fn = /\W/.test(fn = templateString) ? new Function("obj", "var p=[],print=function(){p.push.apply(p,arguments);};with(obj){p.push('" + fn.replace(/[\r\t\n]/g, " ").split("<%").join("\t").replace(/((^|%>)[^\t]*)'/g, "$1\r").replace(/\t=(.*?)%>/g, "',$1,'").split("\t").join("');").split("%>").join("p.push('").split("\r").join("\\'") + "');}return p.join('');") : cache[fn], data ? fn(data) : fn;
  }),
    ...
    easeInElastic: function easeInElastic(t) {
      var s = 1.70158,
          p = 0,
          a = 1;
      return 0 === t ? 0 : 1 == (t /= 1) ? 1 : (p = p || .3, s = a < Math.abs(1) ? (a = 1, p / 4) : p / (2 * Math.PI) * Math.asin(1 / a), -(a * Math.pow(2, 10 * --t) * Math.sin((+t - s) * (2 * Math.PI) / p)));
    },
    easeOutElastic: function easeOutElastic(t) {
      var s = 1.70158,
          p = 0,
          a = 1;
      return 0 === t ? 0 : 1 == (t /= 1) ? 1 : (p = p || .3, s = a < Math.abs(1) ? (a = 1, p / 4) : p / (2 * Math.PI) * Math.asin(1 / a), a * Math.pow(2, -10 * t) * Math.sin((+t - s) * (2 * Math.PI) / p) + 1);
    },
    easeInOutElastic: function easeInOutElastic(t) {
      var s = 1.70158,
          p = 0,
          a = 1;
      return 0 === t ? 0 : 2 == (t /= .5) ? 1 : (p = p || .3 * 1.5 * 1, s = a < Math.abs(1) ? (a = 1, p / 4) : p / (2 * Math.PI) * Math.asin(1 / a), t < 1 ? a * Math.pow(2, 10 * --t) * Math.sin((+t - s) * (2 * Math.PI) / p) * -.5 : a * Math.pow(2, -10 * --t) * Math.sin((+t - s) * (2 * Math.PI) / p) * .5 + 1);
    },
    ...
    var mouseX,
        ...
        mouseY = (mouseY.currentTarget || mouseY.srcElement).getBoundingClientRect(),
        mouseY = e.touches ? (mouseX = e.touches[0].clientX - mouseY.left, e.touches[0].clientY - mouseY.top) : (mouseX = e.clientX - mouseY.left, e.clientY - mouseY.top);
    return {
      x: mouseX,
      y: mouseY
    };
    ...
    return el.currentStyle ? el.currentStyle[property] : document.defaultView.getComputedStyle(el, null).getPropertyValue(property);
      ...
      retinaScale = (helpers.getMaximumSize = helpers.getMaximumWidth, helpers.retinaScale = function (height) {
    var ctx = height.ctx,
    ...
    ctx.font = font;
    var longest = 0;
    return each(arrayOfStrings, function (textWidth) {
  ...
  Chart.instances = {}, extend((Chart.Type = function (data, options, chart) {
    ...
    reflow: noop,
    render: function render(animation) {
      return animation && this.reflow(), this.options.animation && !animation ? ((animation = new Chart.Animation()).numSteps = this.options.animationSteps, animation.easing = this.options.animationEasing, animation.render = function (chartInstance, animationObject) {
        var easeDecimal = helpers.easingEffects[animationObject.easing],
            stepDecimal = animationObject.currentStep / animationObject.numSteps,
            easeDecimal = easeDecimal(stepDecimal);
        chartInstance.draw(easeDecimal, stepDecimal, animationObject.currentStep);
      }, animation.onAnimationProgress = this.options.onAnimationProgress, animation.onAnimationComplete = this.options.onAnimationComplete, Chart.animationService.addAnimation(this, animation)) : (this.draw(), this.options.onAnimationComplete.call(this)), this;
    },
    ...
    showTooltip: function showTooltip(ChartElements, medianPosition) {
      if (void 0 === this.activeElements && (this.activeElements = []), function (Elements) {
        ...
        if (this.activeElements = ChartElements, this.draw(), this.options.customTooltips && this.options.customTooltips(!1), 0 < ChartElements.length) if (this.datasets && 1 < this.datasets.length) {
    ...
    return ChartType.prototype = clone(parent.prototype), extend(ChartType.prototype, extensions), ChartType.extend = Chart.Type.extend, extensions.name || parent.prototype.name ? (chartName = extensions.name || parent.prototype.name, baseDefaults = Chart.defaults[parent.prototype.name] ? clone(Chart.defaults[parent.prototype.name]) : {}, Chart.defaults[chartName] = extend(baseDefaults, extensions.defaults), Chart.types[chartName] = ChartType, Chart.prototype[chartName] = function (data, config) {
      config = merge(Chart.defaults.global, Chart.defaults[chartName], config || {});
      return new ChartType(data, config, this);
    }) : warn("Name not provided for this chart, so it hasn't been registered"), parent;
    ...
    draw: function draw() {
      var ctx;
      this.display && ((ctx = this.ctx).beginPath(), ctx.arc(this.x, this.y, this.radius, 0, 2 * Math.PI), ctx.closePath(), ctx.strokeStyle = this.strokeColor, ctx.lineWidth = this.strokeWidth, ctx.fillStyle = this.fillColor, ctx.fill(), ctx.stroke());
    }
  }), Chart.Arc = Chart.Element.extend({
    inRange: function inRange(startAngle, betweenAngles) {
      var withinRadius = helpers.getAngleFromPoint(this, {
        ...
        y: betweenAngles
          ...
          betweenAngles = betweenAngles < startAngle ? pointRelativeAngle <= betweenAngles || startAngle <= pointRelativeAngle : startAngle <= pointRelativeAngle && pointRelativeAngle <= betweenAngles,
          withinRadius = withinRadius.distance >= this.innerRadius && withinRadius.distance <= this.outerRadius;
    ...
    draw: function draw() {
      ...
      if (ctx.fillStyle = this.fillColor, this.custom) this.custom(this);else {
        switch (this.yAlign) {
          case "above":
            ctx.beginPath(), ctx.moveTo(this.x, this.y - 2), ctx.lineTo(this.x + this.caretHeight, this.y - (2 + this.caretHeight)), ctx.lineTo(this.x - this.caretHeight, this.y - (2 + this.caretHeight)), ctx.closePath(), ctx.fill();
            break;

          case "below":
            tooltipY = this.y + 2 + this.caretHeight, ctx.beginPath(), ctx.moveTo(this.x, this.y + 2), ctx.lineTo(this.x + this.caretHeight, this.y + 2 + this.caretHeight), ctx.lineTo(this.x - this.caretHeight, this.y + 2 + this.caretHeight), ctx.closePath(), ctx.fill();
        }
    ...
    draw: function draw() {
      var ctx;
      this.custom ? this.custom(this) : (drawRoundedRectangle(this.ctx, this.x, this.y - this.height / 2, this.width, this.height, this.cornerRadius), (ctx = this.ctx).fillStyle = this.fillColor, ctx.fill(), ctx.closePath(), ctx.textAlign = "left", ctx.textBaseline = "middle", ctx.fillStyle = this.titleTextColor, ctx.font = this.titleFont, ctx.fillText(this.title, this.x + this.xPadding, this.getLineHeight(0)), ctx.font = this.font, helpers.each(this.labels, function (label, index) {
    ...
    buildYLabels: function buildYLabels() {
      ...
      for (var stepDecimalPlaces = getDecimalPlaces(this.stepValue), i = 0; i <= this.steps; i++) {
    ...
    fit: function fit() {
      ...
      for (this.calculateYRange(cachedHeight), this.buildYLabels(), this.calculateXLabelRotation(); cachedHeight > this.endPoint - this.startPoint;) {
        cachedHeight = this.endPoint - this.startPoint, cachedYLabelWidth = this.yLabelWidth, this.calculateYRange(cachedHeight), this.buildYLabels(), cachedYLabelWidth < this.yLabelWidth && (this.endPoint = cachedEndPoint, this.calculateXLabelRotation());
      }
    },
    calculateXLabelRotation: function calculateXLabelRotation() {
      ...
      if (this.xScalePaddingRight = lastWidth / 2 + 3, this.xScalePaddingLeft = firstWidth / 2 > this.yLabelWidth ? firstWidth / 2 : this.yLabelWidth, this.xLabelRotation = 0, this.display) {
        ...
        for (var xGridWidth = Math.floor(this.calculateX(1) - this.calculateX(0)) - 6; this.xLabelWidth > xGridWidth && 0 === this.xLabelRotation || this.xLabelWidth > xGridWidth && this.xLabelRotation <= 90 && 0 < this.xLabelRotation;) {
          cosRotation = Math.cos(toRadians(this.xLabelRotation)), (firstRotated = cosRotation * firstWidth) + this.fontSize / 2 > this.yLabelWidth && (this.xScalePaddingLeft = firstRotated + this.fontSize / 2), this.xScalePaddingRight = this.fontSize / 2, this.xLabelRotation++, this.xLabelWidth = cosRotation * originalLabelWidth;
        }
    ...
    calculateX: function calculateX(valueOffset) {
      this.xLabelRotation;
    ...
    setScaleSize: function setScaleSize() {
      ...
      for (this.ctx.font = fontString(this.pointLabelFontSize, this.pointLabelFontStyle, this.pointLabelFontFamily), i = 0; i < this.valuesCount; i++) {
        pointPosition = this.getPointPosition(i, largestPossibleRadius), textWidth = this.ctx.measureText(template(this.templateString, {
          value: this.labels[i]
    ...
    setCenterPoint: function setCenterPoint(maxLeft, maxRight) {
      maxRight = this.width - maxRight - this.drawingArea, maxLeft += this.drawingArea;
      this.xCenter = (maxLeft + maxRight) / 2, this.yCenter = this.height / 2;
    },
    getIndexAngle: function getIndexAngle(index) {
      return index * (2 * Math.PI / this.valuesCount) - Math.PI / 2;
    },
    getPointPosition: function getPointPosition(thisAngle, distanceFromCenter) {
      ...
      if (this.display) {
        ...
        if (each(this.yLabels, function (label, index) {
          if (0 < index) {
            ...
            if (0 < this.lineWidth) if (ctx.strokeStyle = this.lineColor, ctx.lineWidth = this.lineWidth, this.lineArc) ctx.beginPath(), ctx.arc(this.xCenter, this.yCenter, labelWidth, 0, 2 * Math.PI), ctx.closePath(), ctx.stroke();else {
              ctx.beginPath();
            ...
            this.showLabels && (ctx.font = fontString(this.fontSize, this.fontStyle, this.fontFamily), this.showLabelBackdrop && (labelWidth = ctx.measureText(label).width, ctx.fillStyle = this.backdropColor, ctx.fillRect(this.xCenter - labelWidth / 2 - this.backdropPaddingX, yHeight - this.fontSize / 2 - this.backdropPaddingY, labelWidth + 2 * this.backdropPaddingX, this.fontSize + 2 * this.backdropPaddingY)), ctx.textAlign = "center", ctx.textBaseline = "middle", ctx.fillStyle = this.fontColor, ctx.fillText(label, this.xCenter, yHeight));
          }
        }, this), !this.lineArc) {
          ...
          for (var i = this.valuesCount - 1; 0 <= i; i--) {
            ...
            }, ctx.beginPath(), ctx.moveTo(this.xCenter, this.yCenter), ctx.lineTo(halfLabelsCount.x, halfLabelsCount.y), ctx.lineTo(exactQuarter.x, exactQuarter.y), ctx.lineTo(upperHalf.x, upperHalf.y), ctx.fillStyle = this.backgroundColors[i], ctx.fill(), ctx.closePath());
            ...
            var labelsCount = this.labels.length,
                ...
                exactQuarter = i === exactQuarter || i === labelsCount - exactQuarter;
            ctx.textAlign = 0 === i || i === halfLabelsCount ? "center" : i < halfLabelsCount ? "left" : "right", ctx.textBaseline = exactQuarter ? "middle" : upperHalf ? "bottom" : "top", ctx.fillText(this.labels[i], pointLabelPosition.x, pointLabelPosition.y);
          }
        }
      }
    }
    ...
    startDigest: function startDigest() {
      ...
      for (var i = 0; i < this.animations.length; i++) {
        null === this.animations[i].animationObject.currentStep && (this.animations[i].animationObject.currentStep = 0), this.animations[i].animationObject.currentStep += 1 + framesToDrop, this.animations[i].animationObject.currentStep > this.animations[i].animationObject.numSteps && (this.animations[i].animationObject.currentStep = this.animations[i].animationObject.numSteps), this.animations[i].animationObject.render(this.animations[i].chartInstance, this.animations[i].animationObject), this.animations[i].animationObject.currentStep == this.animations[i].animationObject.numSteps && (this.animations[i].animationObject.onAnimationComplete.call(this.animations[i].chartInstance), this.animations.splice(i, 1), i--);
      }

      frameDelay = (Date.now() - frameDelay - this.frameDuration) / this.frameDuration;
    ...
    return Chart;
  }) : "object" == (typeof module === "undefined" ? "undefined" : _typeof(module)) && module.exports && (module.exports = Chart), (root.Chart = Chart).noConflict = function () {
  ...
  Chart.Type.extend({
    ...
    initialize: function initialize(data) {
      ...
      this.ScaleClass = Chart.Scale.extend({
        offsetGridLines: !0,
        calculateBarX: function calculateBarX(barWidth, datasetIndex, barIndex) {
        ...
        calculateBarWidth: function calculateBarWidth(datasetCount) {
          return (this.calculateBaseWidth() - (datasetCount - 1) * options.barDatasetSpacing) / datasetCount;
        }
      }), this.datasets = [], this.options.showTooltips && helpers.bindEvents(this, this.options.tooltipEvents, function (activeBars) {
        activeBars = "mouseout" !== activeBars.type ? this.getBarsAtEvent(activeBars) : [];
        this.eachBars(function (bar) {
          bar.restore(["fillColor", "strokeColor"]);
        }), helpers.each(activeBars, function (activeBar) {
          activeBar && (activeBar.fillColor = activeBar.highlightFill, activeBar.strokeColor = activeBar.highlightStroke);
        ...
        this.datasets.push(datasetObject), helpers.each(dataset.data, function (dataPoint, index) {
          datasetObject.bars.push(new this.BarClass({
            ...
            strokeColor: "object" == _typeof(dataset.strokeColor) ? dataset.strokeColor[index] : dataset.strokeColor,
            fillColor: "object" == _typeof(dataset.fillColor) ? dataset.fillColor[index] : dataset.fillColor,
            highlightFill: dataset.highlightFill ? "object" == _typeof(dataset.highlightFill) ? dataset.highlightFill[index] : dataset.highlightFill : "object" == _typeof(dataset.fillColor) ? dataset.fillColor[index] : dataset.fillColor,
            highlightStroke: dataset.highlightStroke ? "object" == _typeof(dataset.highlightStroke) ? dataset.highlightStroke[index] : dataset.highlightStroke : "object" == _typeof(dataset.strokeColor) ? dataset.strokeColor[index] : dataset.strokeColor
          }));
        }, this);
    ...
    buildScale: function buildScale(scaleOptions) {
      var self = this,
          scaleOptions = {
        ...
        calculateYRange: function calculateYRange(updatedRanges) {
          var values,
              updatedRanges = helpers.calculateScaleRange((values = [], self.eachBars(function (bar) {
            values.push(bar.value);
          }), values), updatedRanges, this.fontSize, this.beginAtZero, this.integersOnly);
        ...
        padding: !this.options.showScale && this.options.barShowStroke ? this.options.barStrokeWidth : 0,
    ...
    draw: function draw(ease) {
      ...
      this.clear();
      this.chart.ctx;
      this.scale.draw(easingDecimal), helpers.each(this.datasets, function (dataset, datasetIndex) {
        helpers.each(dataset.bars, function (bar, index) {
          bar.hasValue() && (bar.base = this.scale.endPoint, bar.transition({
            x: this.scale.calculateBarX(this.datasets.length, datasetIndex, index),
  ...
  Chart.Type.extend({
    ...
    initialize: function initialize(data) {
      ...
      }), this.options.showTooltips && helpers.bindEvents(this, this.options.tooltipEvents, function (activeSegments) {
        activeSegments = "mouseout" !== activeSegments.type ? this.getSegmentsAtEvent(activeSegments) : [];
    ...
    addData: function addData(segment, index, silent) {
    ...
    removeData: function removeData(indexToDelete) {
      indexToDelete = helpers.isNumber(indexToDelete) ? indexToDelete : this.segments.length - 1;
  ...
  Chart.Type.extend({
    ...
    initialize: function initialize(data) {
      ...
      }), this.datasets = [], this.options.showTooltips && helpers.bindEvents(this, this.options.tooltipEvents, function (activePoints) {
        activePoints = "mouseout" !== activePoints.type ? this.getPointsAtEvent(activePoints) : [];
    ...
    buildScale: function buildScale(scaleOptions) {
      var self = this,
        ...
        calculateYRange: function calculateYRange(updatedRanges) {
          var values,
              updatedRanges = helpers.calculateScaleRange((values = [], self.eachPoints(function (point) {
    ...
    draw: function draw(ease) {
      ...
      var ctx = this.chart.ctx;
      this.scale && (this.scale.draw(easingDecimal), helpers.each(this.datasets, function (dataset) {
        var pointsWithValues = helpers.where(dataset.points, hasValue);
        helpers.each(dataset.points, function (point, index) {
          point.hasValue() && point.transition({
            y: this.scale.calculateY(point.value),
            x: this.scale.calculateX(index)
          }, easingDecimal);
        }, this), this.options.bezierCurve && helpers.each(pointsWithValues, function (point, index) {
          var tension = 0 < index && index < pointsWithValues.length - 1 ? this.options.bezierCurveTension : 0;
          point.controlPoints = helpers.splineCurve(previousPoint(point, pointsWithValues, index), point, function (point, collection, index) {
        ...
        }, this), ctx.lineWidth = this.options.datasetStrokeWidth, ctx.strokeStyle = dataset.strokeColor, ctx.beginPath(), helpers.each(pointsWithValues, function (point, previous) {
          0 === previous ? ctx.moveTo(point.x, point.y) : this.options.bezierCurve ? (previous = previousPoint(point, pointsWithValues, previous), ctx.bezierCurveTo(previous.controlPoints.outer.x, previous.controlPoints.outer.y, point.controlPoints.inner.x, point.controlPoints.inner.y, point.x, point.y)) : ctx.lineTo(point.x, point.y);
        }, this), this.options.datasetStroke && ctx.stroke(), this.options.datasetFill && 0 < pointsWithValues.length && (ctx.lineTo(pointsWithValues[pointsWithValues.length - 1].x, this.scale.endPoint), ctx.lineTo(pointsWithValues[0].x, this.scale.endPoint), ctx.fillStyle = dataset.fillColor, ctx.closePath(), ctx.fill()), helpers.each(pointsWithValues, function (point) {
          point.draw();
        });
      }, this));
    }
  ...
  Chart.Type.extend({
    ...
    initialize: function initialize(data) {
      this.segments = [], this.SegmentArc = Chart.Arc.extend({
        showStroke: this.options.segmentShowStroke,
        strokeWidth: this.options.segmentStrokeWidth,
        strokeColor: this.options.segmentStrokeColor,
        ctx: this.chart.ctx,
      ...
      }, this), this.options.showTooltips && helpers.bindEvents(this, this.options.tooltipEvents, function (activeSegments) {
        activeSegments = "mouseout" !== activeSegments.type ? this.getSegmentsAtEvent(activeSegments) : [];
        helpers.each(this.segments, function (segment) {
          segment.restore(["fillColor"]);
        }), helpers.each(activeSegments, function (activeSegment) {
    ...
    updateScaleRange: function updateScaleRange(scaleSizes) {
      var valuesArray = [];
    ...
    reflow: function reflow() {
      helpers.extend(this.SegmentArc.prototype, {
        x: this.chart.width / 2,
        y: this.chart.height / 2
      }), this.updateScaleRange(this.segments), this.scale.update(), helpers.extend(this.scale, {
        xCenter: this.chart.width / 2,
        yCenter: this.chart.height / 2
      }), helpers.each(this.segments, function (segment) {
        segment.update({
    ...
    draw: function draw(ease) {
      ...
      this.clear(), helpers.each(this.segments, function (segment, index) {
        segment.transition({
          ...
          outerRadius: this.scale.calculateCenterOffset(segment.value)
        }, easingDecimal), segment.endAngle = segment.startAngle + segment.circumference, 0 === index && (segment.startAngle = 1.5 * Math.PI), index < this.segments.length - 1 && (this.segments[index + 1].startAngle = segment.endAngle), segment.draw();
  ...
  Chart.Type.extend({
    ...
    initialize: function initialize(data) {
      this.PointClass = Chart.Point.extend({
        ...
        hitDetectionRadius: this.options.pointHitDetectionRadius,
        ctx: this.chart.ctx
      }), this.datasets = [], this.buildScale(data), this.options.showTooltips && helpers.bindEvents(this, this.options.tooltipEvents, function (activePointsCollection) {
        activePointsCollection = "mouseout" !== activePointsCollection.type ? this.getPointsAtEvent(activePointsCollection) : [];
        this.eachPoints(function (point) {
          point.restore(["fillColor", "strokeColor"]);
        ...
        this.datasets.push(datasetObject), helpers.each(dataset.data, function (dataPoint, index) {
          var pointPosition;
          this.scale.animation || (pointPosition = this.scale.getPointPosition(index, this.scale.calculateCenterOffset(dataPoint))), datasetObject.points.push(new this.PointClass({
            value: dataPoint,
            label: data.labels[index],
            datasetLabel: dataset.label,
    ...
    getPointsAtEvent: function getPointsAtEvent(fromCenter) {
      ...
      return (pointIndex >= this.scale.valuesCount || pointIndex < 0) && (pointIndex = 0), fromCenter.distance <= this.scale.drawingArea && helpers.each(this.datasets, function (dataset) {
    ...
    buildScale: function buildScale(data) {
      ...
      }), this.scale.setScaleSize(), this.updateScaleRange(data.datasets), this.scale.buildYLabels();
    },
    updateScaleRange: function updateScaleRange(scaleSizes) {
      var totalDataArray,
          scaleSizes = (totalDataArray = [], helpers.each(scaleSizes, function (dataset) {
        ...
        min: this.options.scaleStartValue,
        max: this.options.scaleStartValue + this.options.scaleSteps * this.options.scaleStepWidth
      } : helpers.calculateScaleRange(scaleSizes, helpers.min([this.chart.width, this.chart.height]) / 2, this.options.scaleFontSize, this.options.scaleBeginAtZero, this.options.scaleIntegersOnly);
      helpers.extend(this.scale, scaleSizes);
    },
    addData: function addData(valuesArray, label) {
    ...
    draw: function draw(ease) {
      var easeDecimal = ease || 1,
          ctx = this.chart.ctx;
      this.clear(), this.scale.draw(), helpers.each(this.datasets, function (dataset) {
        helpers.each(dataset.points, function (point, index) {
        ...
        }, this), ctx.closePath(), ctx.stroke(), ctx.fillStyle = dataset.fillColor, this.options.datasetFill && ctx.fill(), helpers.each(dataset.points, function (point) {
