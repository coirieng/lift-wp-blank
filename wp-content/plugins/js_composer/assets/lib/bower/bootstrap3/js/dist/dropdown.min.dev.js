...
!function ($) {
  ...
  Dropdown.VERSION = "3.1.1", Dropdown.prototype.toggle = function (e) {
    ...
    if (/(38|40|27)/.test(e.keyCode)) {
      ...
      if (e.preventDefault(), e.stopPropagation(), !index.is(".disabled, :disabled")) {
        var $items = getParent(index),
            isActive = $items.hasClass("open");
        if (!isActive || isActive && 27 == e.keyCode) return 27 == e.which && $items.find(toggle).trigger("focus"), index.trigger("click");
        index = " li:not(.divider):visible a", $items = $items.find('[role="menu"]' + index + ', [role="listbox"]' + index);
        $items.length && (index = $items.index($items.filter(":focus")), 38 == e.keyCode && 0 < index && index--, 40 == e.keyCode && index < $items.length - 1 && index++, ~index || (index = 0), $items.eq(index).trigger("focus"));
      }
    }
  };
  var old = $.fn.dropdown;
