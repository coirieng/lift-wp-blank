...
!function ($) {
  ...
  Button.VERSION = "3.1.1", Button.DEFAULTS = {
    ...
    var d = "disabled",
        ...
        data = $el.data();
    state += "Text", null == data.resetText && $el.data("resetText", $el[val]()), $el[val]((null == data[state] ? this.options : data)[state]), setTimeout($.proxy(function () {
    ...
    $parent.length && ("radio" == ($input = this.$element.find("input")).prop("type") && ($input.prop("checked") && this.$element.hasClass("active") ? changed = !1 : $parent.find(".active").removeClass("active")), changed && $input.prop("checked", !this.$element.hasClass("active")).trigger("change")), changed && this.$element.toggleClass("active");
  };
  var old = $.fn.button;
