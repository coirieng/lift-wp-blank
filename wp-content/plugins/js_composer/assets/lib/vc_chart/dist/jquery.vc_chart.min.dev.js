...
!function ($) {
  ...
  VcChart.prototype = {
    ...
    draw: function draw(redraw) {
      var w = this.$el.addClass("vc_ready").width(),
          radius = (w = (w = w || this.$el.parents(":visible").first().width() - 2) / 100 * 80) / 2 - 5 - 1;
      this.$wrapper.css({
        width: w + "px"
      }), this.$label.css({
        width: w,
    ...
    setProgress: function setProgress() {
      ...
      var val = this._progress_v / this.value * this.label_value,
          val = Math.round(val) + this.options.units;
    ...
    setWayPoint: function setWayPoint() {
