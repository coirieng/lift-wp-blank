...
jQuery(document).ready(function ($) {
  ...
  $(document).on('click', '.rsssl-copy-content', function () {
    ...
    if (success) {
      ...
      setTimeout(function () {
        $('.rsssl-copied-feedback-container').fadeOut('slow');
      }, 5000);
    }
  });
  ...
  function check_conditions() {
    ...
    $(".condition-check-1").each(function (e) {
      ...
      for (i = 1; i < 4; i++) {
        ...
        var condition_answer = $(this).data("condition-answer-" + i); //remove required attribute of child, and set a class.
        ...
        condition_answers.forEach(function (condition_answer) {
          ...
          if (showIfConditionMet) {
            ...
            if (conditionMet || value.indexOf(condition_answer) != -1 || value == condition_answer || condition_answer === 'EMPTY' && value == '') {
              container.removeClass("rsssl-hidden"); //remove required attribute of child, and set a class.

              if (input.hasClass('is-required')) input.prop('required', true); //prevent further checks if it's an or/and statement
