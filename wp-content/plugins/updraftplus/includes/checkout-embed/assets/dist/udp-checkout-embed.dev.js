    ...
    modal: {
      open: function open($target) {
        ...
        if (!this.product_url || this.product_url && this.product_url != $target.data('embed-checkout')) {
          this.product_url = $target.data('embed-checkout'); // Disabled because of SameSite cookie issues
          // this.setup();
        }
        ...
        if (0) {
          ...
          this.$el.removeClass('iframe-is-opened'); // Show it.
      ...
      setup: function setup() {
        ...
        this.$el = $(template); // receives events from iframe
