...
(function ($) {
  ...
  $.widget("ui.dialog", $.ui.dialog, {
    options: {
      height: 200,
      // auto is not allowed when using animation
    ...
    change: function change(content, width, height, animate) {
    ...
    _calcSize: function _calcSize(data, value, sortBy, toSort) {
      var newData = {};
    ...
    _animateUsing: function _animateUsing(position, data) {
      var that = this; // calculate new position based on the viewport
    ...
    _position: function _position() {
      var that = this;
      this._positionOptions = this.options.position; // change position.using mechanism
      ...
      this._super(); // reset position.using mechanism


      this.options.position = this._positionOptions;
    },
    // ARIA helper
    setAriaLive: function setAriaLive(busy) {
      this.uiDialog.attr({
        "aria-live": "assertive",
    ...
    _addTimeout: function _addTimeout(callback) {
