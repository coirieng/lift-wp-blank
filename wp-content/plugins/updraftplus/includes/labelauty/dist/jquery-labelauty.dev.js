...
(function ($) {
  $.fn.labelauty = function (options) {
    ...
    return this.each(function () {
      ...
      var labels;
      var labels_object;
      var input_id; //Get the aria label from the input element

      var aria_label = $object.attr("aria-label"); // Hide the object form screen readers

      $object.attr("aria-hidden", true); // Test if object is a check input
      // Don't mess me up, come on

      if ($object.is(":checkbox") === false && $object.is(":radio") === false) return this; // Add "labelauty" class to all checkboxes
      // So you can apply some custom styles

      $object.addClass(settings["class"]); // Get the value of "data-labelauty" attribute
      // Then, we have the labels for each case (or not, as we will see)

      labels = $object.attr("data-labelauty");
      use_labels = settings.label; // It's time to check if it's going to the right way
      ...
      if (use_labels === true) {
        if (labels == null || labels.length === 0) {
          ...
          labels_object = labels.split(settings.separator); // Now, let's check if exist _only_ two labels
      ...
      $object.removeAttr("data-labelauty"); // Now, grab checkbox ID Attribute for "label" tag use
      ...
      if (settings.force_random_id || input_id == null || input_id.trim() === "") {
        ...
        input_id = "labelauty-" + input_id_number; // Is there any element with this random ID ?
      ...
      $object.after(element); // Now, add "min-width" to label
      // Let's say the truth, a fixed width is more beautiful than a variable width

      if (settings.minimum_width !== false) $object.next("label[for=" + input_id + "]").css({
  ...
  function create(input_id, aria_label, selected, type, messages_object, label) {
    ...
    if (messages_object == null) unchecked_message = checked_message = "";else {
      unchecked_message = messages_object[0]; // If checked message is null, then put the same text of unchecked message

      if (messages_object[1] == null) checked_message = unchecked_message;else checked_message = messages_object[1];
    }
    if (aria_label == null) aria = "";else aria = 'tabindex="0" role="' + type + '" aria-checked="' + selected + '" aria-label="' + aria_label + '"';
