...
jQuery(function ($) {
  ...
  function updraft_restore_command(job_id, action) {
    ...
    xhttp.onprogress = function (response) {
      if (response.currentTarget.status >= 200 && response.currentTarget.status < 300) {
        ...
        var i = 0;
        var end_of_json = 0; // Check if there is restore information json in the response if so process it and remove it from the response so that it does not make it to page
        ...
        while (i < responseText.length) {
          ...
          if ('RINFO:{' == buffer) {
            ...
            $output.append(responseText.substring(end_of_json, i).trim()) // add the text to the activity log
            ...
            if (1 == debug) {
              console.log(analyse_it);
            }

            updraft_restore_process_data(analyse_it.parsed); // move the for loop counter to the end of the json

            end_of_json = i + analyse_it.json_last_pos - analyse_it.json_start_pos + 6; // When the for loop goes round again, it will start with the end of the JSON
        ...
        $output.append(responseText.substr(end_of_json).trim()).scrollTop($output[0].scrollHeight); // check if the fylesystem form is displayed
    ...
    xhttp.onload = function () {
      var $result = $output.find('.updraft_restore_successful, .updraft_restore_error'); // if we don't find the result, exit
  ...
  function updraft_restore_process_data(restore_data) {
    ...
    if (restore_data) {
      if ('state' == restore_data.type || 'state_change' == restore_data.type) {
        ...
        var $current = $steps_list.find('[data-component=' + current_stage + ']'); // show simplified activity log next to the component's label
        ...
        if (previous_stage !== current_stage) {
          if (previous_stage) {
            var $prev = $steps_list.find('[data-component=' + previous_stage + ']'); // empty the line's status
          ...
          if ('finished' == current_stage) {
            ...
            if (restore_data.data.hasOwnProperty('actions') && 'object' == _typeof(restore_data.data.actions)) {
  ...
  function updraft_restore_resume_notice() {
    updraft_send_command('get_restore_resume_notice', {
      job_id: job_id
    }, function (response) {
      ...
      error_callback: function error_callback(response, status, error_code, resp) {
        if (500 == response.status && 3 > server_500_count) {
          server_500_count++;
          updraft_restore_command(job_id, 'updraft_ajaxrestore_continue');
        } else {
          updraft_restore_process_data({
            stage: 'finished',
            type: 'state_change'
          });
  ...
  $(document).on('heartbeat-tick', function (event, heartbeat_data) {
    if (!heartbeat_data.hasOwnProperty('wp-auth-check')) return; // check if we are logged out
    ...
    var updraftplus_data = heartbeat_data.updraftplus; // if we are logged in, check if theres a new nonce
