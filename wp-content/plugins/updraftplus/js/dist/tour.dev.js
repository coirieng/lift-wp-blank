...
(function ($) {
  $(function () {
    ...
    */
    ...
    var main_tour = window.updraft_main_tour = new Shepherd.Tour(); // Set up the defaults for each step
    ...
    plugins_page_tour.addStep('intro', {
      ...
      when: {
        show: function show() {
          ...
          var popup = $(this.el); // var target = $(this.tether.target);
    ...
    */
    ...
    main_tour.steps.forEach(function (step) {
      step.once('show', function () {
        ...
        close_btn.attr('data-btntext', updraftplus_tour_i18n.close); // opens the settings tab
    ...
    function cancel_tour() {
      ...
      if ('function' === typeof updraft_send_command) {
        updraft_send_command('set_tour_status', data, function (response) {
    ...
    var scroll_to_popup = function scroll_to_popup(step) {
    ...
    if ($('#updraft-backupnow-button').length) {
      ...
      */
      ...
      main_tour.start(); // go back to fisrt tab
