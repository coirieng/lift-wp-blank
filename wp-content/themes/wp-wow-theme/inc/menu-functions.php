...
function wow_add_menu_item_class( $classes, $item, $args, $depth ){
	$classes[] = 'nav-item';
	return $classes;
}
add_filter ( 'nav_menu_css_class', 'wow_add_menu_item_class', 10, 4 );

function wow_add_sub_menu_toggle( $output, $item, $depth, $args ) {
	if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {

		// Add toggle button.
		$output .= '<button class="sub-menu-toggle menu-toggle" aria-expanded="false" onClick="___ExpandSubMenu(this)">';
		$output .= '<span class="icon-plus">' . wow_get_icon_svg( 'ui', 'plus', 18 ) . '</span>';
		$output .= '<span class="icon-minus">' . wow_get_icon_svg( 'ui', 'minus', 18 ) . '</span>';
		$output .= '<span class="screen-reader-text">' . esc_html__( 'Open menu', 'wp-wow-theme' ) . '</span>';
		$output .= '</button>';
	}
	return $output;
}
add_filter( 'walker_nav_menu_start_el', 'wow_add_sub_menu_toggle', 10, 4 );
 ...
 */
function wow_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	// Change SVG icon inside social links menu if there is supported URL.
	if ( 'footer' === $args->theme_location ) {
		$svg = wow_get_social_link_svg( $item->url, 24 );
		if ( ! empty( $svg ) ) {
...
add_filter( 'walker_nav_menu_start_el', 'wow_nav_menu_social_icons', 10, 4 );
 ...
 * @return stdClass
 */
function wow_add_menu_description_args( $args, $item, $depth ) {
	$args->link_after = '';
...
add_filter( 'nav_menu_item_args', 'wow_add_menu_description_args', 10, 3 );
